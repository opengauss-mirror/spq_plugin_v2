...
/* application name used for internal connections in Spq */
constexpr char* SPQ_APPLICATION_NAME_PREFIX = "spq_internal gpid=";

/* application name used for internal connections in rebalancer */
constexpr char* SPQ_REBALANCER_APPLICATION_NAME_PREFIX = "spq_rebalancer gpid=";

/* application name used for connections made by run_command_on_* */
constexpr char* SPQ_RUN_COMMAND_APPLICATION_NAME_PREFIX = "spq_run_command gpid=";
...
struct SessionConnCtx {
    /* the hash tables are externally accessiable */
    HTAB* ConnectionHash{nullptr};

    /* context for all connection and transaction related memory */
    MemoryContext ConnectionContext{nullptr};

    HTAB* ConnectionPlacementHash{nullptr};

    HTAB* ColocatedPlacementsHash{nullptr};

    HTAB* ConnectionShardHash{nullptr};

    /* session specific hash map*/
    HTAB* SessionLocalReservedConnections{nullptr};

    void InitializeConnCtx();
};
...
extern bool CitusModifyWaitEvent(WaitEventSet* set, int pos, uint32 events, Latch* latch);
