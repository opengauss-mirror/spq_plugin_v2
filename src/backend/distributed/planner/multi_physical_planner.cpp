...
static Node* QueryJoinTree(MultiNode* multiNode, List* dependentJobList,
                           List** rangeTableList);
static void SetJoinRelatedColumnsCompat(RangeTblEntry* rangeTableEntry, Oid leftRelId,
                                        Oid rightRelId, List* leftColumnVars,
...
static MapMergeJob* BuildMapMergeJob(Query* jobQuery, List* dependentJobList,
                                     Var* partitionKey,
                                     RemoteJobPartitionType partitionType,
                                     Oid baseRelationId,
...
static List* FragmentCombinationList(List* rangeTableFragmentsList, Query* jobQuery,
                                     List* dependentJobList);
static JoinSequenceNode* JoinSequenceArray(List* rangeTableFragmentsList, Query* jobQuery,
                                           List* dependentJobList);
...
static List* ReorderAndAssignTaskList(List* taskList, ReorderFunction reorderFunction);
 ...
 */
bool ModifyLocalTableJob(Job* job)
{
    if (job == NULL) {
        return false;
    }
...
static Job* BuildJobTree(MultiTreeRoot* multiTree)
{
    ...
    while (parentNode != NULL) {
        ...
        if (boundaryNodeJobType == JOIN_MAP_MERGE_JOB) {
            ...
            RemoteJobPartitionType partitionType = PARTITION_INVALID_FIRST;
            ...
            if (CitusIsA(leftChildNode, MultiPartition)) {
                ...
                MapMergeJob* mapMergeJob =
                    BuildMapMergeJob(jobQuery, dependentJobList, partitionKey,
                                     partitionType, baseRelationId, JOIN_MAP_MERGE_JOB);
            ...
            if (CitusIsA(rightChildNode, MultiPartition)) {
                ...
                MapMergeJob* mapMergeJob =
                    BuildMapMergeJob(jobQuery, NIL, partitionKey, partitionType,
                                     baseRelationId, JOIN_MAP_MERGE_JOB);
...
static Query* BuildJobQuery(MultiNode* multiNode, List* dependentJobList)
{
    ...
    if (extendedOpNodeList != NIL) {
        ...
        targetList = static_cast<List*>(copyObject(extendedOp->targetList));
...
static List* BaseRangeTableList(MultiNode* multiNode)
{
    ...
    while (pendingNodeList != NIL) {
        ...
        CitusNodeTag nodeType = static_cast<CitusNodeTag>(CitusNodeTag(currMultiNode));
 ...
 */
RangeTblEntry* DerivedRangeTableEntry(MultiNode* multiNode, List* columnList,
 ...
 */
List* DerivedColumnNameList(uint32 columnCount, uint64 generatingJobId)
{
    ...
    for (uint32 columnIndex = 0; columnIndex < columnCount; columnIndex++) {
        ...
        auto columnValue = makeString(columnName->data);
...
static List* TargetEntryList(List* expressionList)
{
    ...
    foreach (expressionCell, expressionList) {
        ...
        TargetEntry* targetEntry =
            makeTargetEntry(expression, columnNumber, columnName->data, false);
 ...
 * right result.
 */
Node* WrapUngroupedVarsInAnyValueAggregate(Node* expression, List* groupClauseList,
                                           List* targetList, bool checkExpressionEquality)
{
    ...
    Node* result = expression_tree_mutator(
        expression, reinterpret_cast<Node* (*)(Node*, void*)>(AddAnyValueAggregates),
        &context);
...
static Node* AddAnyValueAggregates(Node* node, AddAnyValueAggregatesContext* context)
{
    ...
    return expression_tree_mutator(
        node, reinterpret_cast<Node* (*)(Node*, void*)>(AddAnyValueAggregates), context);
}
...
static Node* QueryJoinTree(MultiNode* multiNode, List* dependentJobList,
                           List** rangeTableList)
{
    CitusNodeTag nodeType = static_cast<CitusNodeTag>(CitusNodeTag(multiNode));
    ...
    switch (nodeType) {
        case T_MultiJoin: {
            ...
            RangeTblEntry* rangeTableEntry =
                JoinRangeTableEntry(joinExpr, dependentJobList, *rangeTableList);
        ...
        case T_MultiTable: {
            ...
            if (unaryNode->childNode != NULL) {
                ...
                Node* childNode =
                    QueryJoinTree(unaryNode->childNode, dependentJobList, rangeTableList);
        ...
        case T_MultiCollect: {
            ...
            List* columnNameList =
                DerivedColumnNameList(columnCount, dependentJob->jobId);
            ...
            RangeTblEntry* rangeTableEntry = DerivedRangeTableEntry(
                multiNode, columnNameList, tableIdList, funcColumnNames, funcColumnTypes,
                                          ...
                                          List* rangeTableList)
{
    ...
    ExtractColumns(leftCallingRTE, leftRangeTableId, &leftColumnNames, &leftColumnVars);
    ExtractColumns(rightCallingRte, rightRangeTableId, &rightColumnNames,
                   &rightColumnVars);
...
/*
 * GetColumnOriginalIndexes gets the original indexes of columns by taking column drops
 * into account.
 */
static List* GetColumnOriginalIndexes(Oid relationId)
{
...
static List* QueryFromList(List* rangeTableList)
{
    ...
    for (Index rangeTableIndex = 1; rangeTableIndex <= rangeTableCount;
         rangeTableIndex++) {
 ...
 * and calls UpdateColumnAttributes to updates the column's range table reference (varno)
 * and column attribute number for the range table (varattno).
 */
static void UpdateAllColumnAttributes(Node* columnContainer, List* rangeTableList,
                                      List* dependentJobList)
{
                                   ...
                                   List* dependentJobList)
{
    ...
    if (GetRangeTblKind(newRangeTableEntry) == CITUS_RTE_REMOTE_QUERY) {
        newColumnId = NewColumnId(originalTableId, originalColumnId, newRangeTableEntry,
                                  dependentJobList);
    }
...
static List* ChildNodeList(MultiNode* multiNode)
{
    ...
    if (isUnaryNode) {
        ...
        MultiBinaryNode* binaryNode = (MultiBinaryNode*)multiNode;
        childNodeList = list_make2(binaryNode->leftChildNode, binaryNode->rightChildNode);
    }
 ...
 *
 * When spq.enable_unique_job_ids is off then only the local counter is
 * included to get repeatable results.
 */
uint64 UniqueJobId(void)
{
    ...
    if (Session_ctx::Vars().EnableUniqueJobIds) {
        /*
         ...
         * Citus.
         */
        processId = t_thrd.proc_cxt.MyProcPid & 0xFFFFFF;
                                     ...
                                     BoundaryNodeJobType boundaryNodeJobType)
{
    ...
    Var* partitionColumn = static_cast<Var*>(copyObject(partitionKey));
    ...
    if (partitionType == DUAL_HASH_PARTITION_TYPE) {
        ...
        ShardInterval** cachedSortedShardIntervalArray = cache->sortedShardIntervalArray;
        bool hasUninitializedShardInterval = cache->hasUninitializedShardInterval;

        ShardInterval** sortedShardIntervalArray =
            static_cast<ShardInterval**>(palloc0(sizeof(ShardInterval) * shardCount));
...
static uint32 HashPartitionCount(void)
{
    ...
    double maxReduceTasksPerNode = Session_ctx::Vars().RepartitionJoinBucketCountPerNode;
                                 ...
                                 PlannerRestrictionContext* plannerRestrictionContext)
{
    ...
    for (int32 jobIndex = (flattenedJobCount - 1); jobIndex >= 0; jobIndex--) {
        ...
        if (job->subqueryPushdown) {
            ...
            List* prunedRelationShardList = TargetShardIntervalsForRestrictInfo(
                plannerRestrictionContext->relationRestrictionContext, &isMultiShardQuery,
                NULL);

            DeferredErrorMessage* deferredErrorMessage = NULL;
            sqlTaskList = QueryPushdownSqlTaskList(
                job->jobQuery, job->jobId,
                plannerRestrictionContext->relationRestrictionContext,
                prunedRelationShardList, READ_TASK, false, &deferredErrorMessage);
            if (deferredErrorMessage != NULL) {
                deferredErrorMessage->Raise(ERROR);
            }
        } else {
 ...
 * to be executed.
 */
List* QueryPushdownSqlTaskList(Query* query, uint64 jobId,
                               ...
                               DeferredErrorMessage** planningError)
{
    ...
    forboth_ptr(prunedShardList, prunedRelationShardList, relationRestriction,
    ...
    while ((shardOffset = bms_next_member(taskRequiredForShardIndex, shardOffset)) >= 0) {
        Task* subqueryTask = QueryPushdownTaskCreate(
            query, shardOffset, relationRestrictionContext, taskIdIndex, taskType,
            modifyRequiresCoordinatorEvaluation, planningError);
    ...
    if (taskType == MODIFY_TASK &&
        list_length(relationRestrictionContext->relationRestrictionList) > 1) {
...
static bool IsInnerTableOfOuterJoin(RelationRestriction* relationRestriction)
{
    ...
    foreach_declared_ptr(joinInfo, relationRestriction->relOptInfo->joininfo)
    {
         ...
         */
        bool isInOuter =
            bms_is_member(relationRestriction->relOptInfo->relid, joinInfo->outer_relids);
        if (!isInOuter) {
...
static void ErrorIfUnsupportedShardDistribution(Query* query)
{
    ...
    foreach (relationIdCell, relationIdList) {
        ...
        if (IsCitusTableType(relationId, RANGE_DISTRIBUTED)) {
            ...
            nonReferenceRelations = lappend_oid(nonReferenceRelations, relationId);
    ...
    } else if ((rangeDistributedRelationCount > 0) &&
               (appendDistributedRelationCount > 0)) {
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg("cannot push down this subquery"),
                        errdetail("A query including both range and append "
                                  "partitioned relations are unsupported")));
    } else if ((appendDistributedRelationCount > 0) &&
               (hashDistOrSingleShardRelCount > 0)) {
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg("cannot push down this subquery"),
    ...
    foreach (relationIdCell, nonReferenceRelations) {
        ...
        bool coPartitionedTables =
            CoPartitionedTables(firstTableRelationId, currentRelationId);
        if (!coPartitionedTables) {
            ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                            errmsg("cannot push down this subquery"),
                            errdetail("A qurery including more than one distributed "
...
static Task* QueryPushdownTaskCreate(Query* originalQuery, int shardIndex,
                                     ...
                                     DeferredErrorMessage** planningError)
{
    Query* taskQuery = static_cast<Query*>(copyObject(originalQuery));
    ...
    foreach (restrictionCell, restrictionContext->relationRestrictionList) {
        ...
        if (!HasDistributionKeyCacheEntry(cacheEntry)) {
            ...
            if (!modifyWithSubselect || relationId == resultRelationOid) {
                /* for UPDATE/DELETE the shard in the result relation becomes the anchor
                 * shard */
                anchorShardId = shardInterval->shardId;
            }
        } else {
 ...
 */
bool CoPartitionedTables(Oid firstRelationId, Oid secondRelationId)
{
...
static List* SqlTaskList(Job* job)
{
     ...
     * (...), (...).
     */
    Node* whereClauseTree = (Node*)make_ands_explicit((List*)jobQuery->jointree->quals);
     ...
     */
    List* rangeTableFragmentsList =
        RangeTableFragmentsList(rangeTableList, whereClauseList, dependentJobList);
     ...
     */
    List* fragmentCombinationList =
        FragmentCombinationList(rangeTableFragmentsList, jobQuery, dependentJobList);
    ...
    foreach (fragmentCombinationCell, fragmentCombinationList) {
        ...
        List* dataFetchTaskList =
            DataFetchTaskList(jobId, taskIdIndex, fragmentCombination);
        ...
        Query* taskQuery = static_cast<Query*>(copyObject(jobQuery));
        ...
        Task* sqlTask =
            CreateBasicTask(jobId, taskIdIndex, READ_TASK, sqlQueryString->data);
        sqlTask->dependentTaskList = dataFetchTaskList;
        sqlTask->relationShardList =
            BuildRelationShardList(fragmentRangeTableList, fragmentCombination);
        ...
        if (anchorRangeTableBasedAssignment) {
            sqlTask->anchorShardId =
                AnchorShardId(fragmentCombination, anchorRangeTableId);
        }
    ...
    foreach (rangeTableCell, rangeTableList) {
        ...
        if (rangeTableKind == CITUS_RTE_RELATION) {
            ...
            List* prunedShardIntervalList =
                PruneShards(relationId, tableId, whereClauseList, NULL);
            ...
            foreach (shardIntervalCell, prunedShardIntervalList) {
                ...
                RangeTableFragment* shardFragment =
                    static_cast<RangeTableFragment*>(palloc0(fragmentSize));
            ...
            rangeTableFragmentsList = lappend(rangeTableFragmentsList, shardFragmentList);
        } else if (rangeTableKind == CITUS_RTE_REMOTE_QUERY) {
            ...
            foreach (mergeTaskCell, mergeTaskList) {
                ...
                RangeTableFragment* mergeTaskFragment =
                    static_cast<RangeTableFragment*>(palloc0(fragmentSize));
            ...
            rangeTableFragmentsList =
                lappend(rangeTableFragmentsList, mergeTaskFragmentList);
        }
 ...
 * built.
 */
Node* BuildBaseConstraint(Var* column)
{
 ...
 * number.
 */
OpExpr* MakeOpExpression(Var* variable, int16 strategyNumber)
{
    ...
    OperatorCacheEntry* operatorCacheEntry =
        LookupOperatorByType(typeId, accessMethodId, strategyNumber);
    ...
    if (operatorClassInputType != typeId && typeType != TYPTYPE_PSEUDO) {
        variable = (Var*)makeRelabelType((Expr*)variable, operatorClassInputType, -1,
                                         collationId, COERCE_IMPLICIT_CAST);
    }

    Const* constantValue = makeNullConst(operatorClassInputType, typeModId, collationId);

    /* Now make the expression with the given variable and a null constant */
    OpExpr* expression = (OpExpr*)make_opclause(
        operatorId, InvalidOid, /* no result type yet */
        false,                  /* no return set */
        (Expr*)variable, (Expr*)constantValue, InvalidOid, collationId);
                                                ...
                                                int16 strategyNumber)
{
    ...
    foreach (cacheEntryCell, OperatorCache) {
        OperatorCacheEntry* cacheEntry =
            static_cast<OperatorCacheEntry*>(lfirst(cacheEntryCell));
    ...
    if (matchingCacheEntry == NULL) {
        ...
        if (u_sess->cache_mem_cxt == NULL) {
            u_sess->cache_mem_cxt = AllocSetContextCreate(
                u_sess->top_mem_cxt, "SessionCacheMemoryContext", ALLOCSET_DEFAULT_SIZES);
            ;
        }
        ...
        matchingCacheEntry =
            static_cast<OperatorCacheEntry*>(palloc0(sizeof(OperatorCacheEntry)));
 ...
 */
bool BinaryOpExpression(Expr* clause, Node** leftOperand, Node** rightOperand)
{
...
Var* MakeInt4Column()
{
    ...
    Var* int4Column = makeVar(tableId, columnAttributeNumber, columnType, columnTypeMod,
                              columnCollationOid, columnLevelSup);
    return int4Column;
}

/* Updates the base constraint with the given min/max values. */
void UpdateConstraint(Node* baseConstraint, ShardInterval* shardInterval)
{
    ...
    minConstant->constvalue = datumCopy(
        shardInterval->minValue, shardInterval->valueByVal, shardInterval->valueTypeLen);
    maxConstant->constvalue = datumCopy(
        shardInterval->maxValue, shardInterval->valueByVal, shardInterval->valueTypeLen);
                                     ...
                                     List* dependentJobList)
{
    ...
    JoinSequenceNode* joinSequenceArray =
        JoinSequenceArray(rangeTableFragmentsList, jobQuery, dependentJobList);
    ...
    while (fragmentCombinationQueue != NIL) {
        ...
        List* fragmentCombination =
            static_cast<List*>(linitial(fragmentCombinationQueue));
        ...
        if (joinSequenceIndex == rangeTableCount) {
            fragmentCombinationList =
                lappend(fragmentCombinationList, fragmentCombination);
            continue;
        }
        ...
        List* tableFragments =
            FindRangeTableFragmentsList(rangeTableFragmentsList, tableId);
        ...
        foreach (tableFragmentCell, tableFragments) {
            RangeTableFragment* tableFragment =
                static_cast<RangeTableFragment*>(lfirst(tableFragmentCell));
            bool joinPrunable = false;

            if (joiningTableId != NON_PRUNABLE_JOIN) {
                RangeTableFragment* joiningTableFragment =
                    static_cast<RangeTableFragment*>(
            ...
            if (!joinPrunable) {
                ...
                fragmentCombinationQueue =
                    lappend(fragmentCombinationQueue, newFragmentCombination);
            }
        }
    }
 ...
 */
static bool NodeIsRangeTblRefReferenceTable(Node* node, List* rangeTableList)
{
 ...
 */
List* FetchEqualityAttrNumsForRTE(Node* node)
{
    if (node == NULL) {
        return NIL;
    }
...
static List* FetchEqualityAttrNumsForList(List* nodeList)
{
    ...
    {
        List* fetchedEqualityAttrNums = FetchEqualityAttrNumsForRTE(node);
                                           ...
                                           List* dependentJobList)
{
    ...
    JoinSequenceNode* joinSequenceArray =
        static_cast<JoinSequenceNode*>(palloc0(rangeTableCount * sequenceNodeSize));
    ...
    foreach (joinExprCell, joinExprList) {
        ...
        bool leftIsReferenceTable =
            NodeIsRangeTblRefReferenceTable(joinExpr->larg, rangeTableList);
        bool rightIsReferenceTable =
            NodeIsRangeTblRefReferenceTable(joinExpr->rarg, rangeTableList);
        bool isReferenceJoin = IsSupportedReferenceJoin(
            joinExpr->jointype, leftIsReferenceTable, rightIsReferenceTable);
        ...
        foreach_declared_ptr(nextJoinClause, nextJoinClauseList)
        {
            ...
            bool leftPartitioned =
                PartitionedOnColumn(leftColumn, rangeTableList, dependentJobList);
            bool rightPartitioned =
                PartitionedOnColumn(rightColumn, rangeTableList, dependentJobList);
 ...
 */
static bool PartitionedOnColumn(Var* column, List* rangeTableList, List* dependentJobList)
{
 ...
 */
static List* FindRangeTableFragmentsList(List* rangeTableFragmentsList, int tableId)
{
                         ...
                         RangeTableFragment* rightFragment)
{
    ...
    if (leftFragment->fragmentType == CITUS_RTE_REMOTE_QUERY &&
        ...
        if (leftMergeTask->partitionId != rightMergeTask->partitionId) {
            ereport(DEBUG2,
                    (errmsg("join prunable for task partitionId %u and %u",
                            leftMergeTask->partitionId, rightMergeTask->partitionId)));
            return true;
        } else {
            return false;
        }
...
bool ShardIntervalsOverlap(ShardInterval* firstInterval, ShardInterval* secondInterval)
{
 ...
 */
bool ShardIntervalsOverlapWithParams(Datum firstMin, Datum firstMax, Datum secondMin,
                                     ...
                                     Oid collation)
{
     ...
     */
    Datum firstDatum =
        FunctionCall2Coll(comparisonFunction, collation, firstMax, secondMin);
    Datum secondDatum =
        FunctionCall2Coll(comparisonFunction, collation, secondMax, firstMin);
 ...
 */
Task* CreateBasicTask(uint64 jobId, uint32 taskId, TaskType taskType, char* queryString)
{
...
static Alias* FragmentAlias(RangeTblEntry* rangeTableEntry, RangeTableFragment* fragment)
{
    ...
    if (fragmentType == CITUS_RTE_RELATION) {
        ...
        Query* fragmentSetQuery = BuildReadIntermediateResultsArrayQuery(
            mapJobTargetList, NIL, resultNameList, useBinaryFormat);

        /* we only really care about the function RTE */
        RangeTblEntry* readIntermediateResultsRTE =
            static_cast<RangeTblEntry*>(linitial(fragmentSetQuery->rtable));
    ...
    ModifyRangeTblExtraData(rangeTableEntry, CITUS_RTE_SHARD, schemaName, fragmentName,
                            NIL);
...
static List* FetchTaskResultNameList(List* mapOutputFetchTaskList)
{
    ...
    {
        Task* mapTask =
            static_cast<Task*>(linitial(mapOutputFetchTask->dependentTaskList));
        ...
        foreach (dependentTaskCell, dependentTaskList) {
            ...
            if (dataFetchTask->taskType == MERGE_FETCH_TASK) {
                ...
                prunedDependendTaskList =
                    lappend(prunedDependendTaskList, mergeTaskReference);

                ereport(DEBUG2,
                        (errmsg("pruning merge fetch taskId %d", dataFetchTask->taskId),
...
static List* MapTaskList(MapMergeJob* mapMergeJob, List* filterTaskList)
{
    ...
    if (groupClauseList != NIL) {
        ...
        partitionColumnResNo =
            PartitionColumnIndex(partitionColumn, filterQuery->targetList);
    }
    ...
    foreach (filterTaskCell, filterTaskList) {
        ...
        StringInfo mapQueryString = CreateMapQueryString(
            mapMergeJob, filterTask, partitionColumnResNo, useBinaryFormat);
                                       ...
                                       uint32 partitionColumnIndex, bool useBinaryFormat)
{
    ...
    RemoteJobPartitionType partitionType = mapMergeJob->partitionType;
    ...
    StringInfo minValuesString = ArrayObjectToString(minValueArray, TEXTOID, InvalidOid);
    StringInfo maxValuesString = ArrayObjectToString(maxValueArray, TEXTOID, InvalidOid);

    const char* partitionMethodString =
        partitionType == RANGE_PARTITION_TYPE ? "range" : "hash";
    ...
    appendStringInfo(mapQueryString,
                     ...
                     quote_literal_cstr(resultNamePrefix),
                     quote_literal_cstr(filterQueryString), partitionColumnIndex - 1,
                     quote_literal_cstr(partitionMethodString), minValuesString->data,
                     maxValuesString->data, useBinaryFormat ? "true" : "false",
 ...
 */
ShardInterval** GenerateSyntheticShardIntervalArray(int partitionCount)
{
    ShardInterval** shardIntervalArray =
        static_cast<ShardInterval**>(palloc0(partitionCount * sizeof(ShardInterval*)));
                                                        ...
                                                        int intervalCount)
{
    ...
    ShardInterval** fullIntervalArray =
        static_cast<ShardInterval**>(palloc0(fullIntervalCount * sizeof(ShardInterval*)));
 ...
 */
RowModifyLevel RowModifyLevelForQuery(Query* query)
{
 ...
 */
StringInfo ArrayObjectToString(ArrayType* arrayObject, Oid columnType,
                               int32 columnTypeMod)
{
    ...
    appendStringInfo(arrayString, "%s::%s", arrayOutputEscapedText, arrayOutTypeName);
                           ...
                           uint32 taskIdIndex)
{
    ...
    for (uint32 partitionId = initialPartitionId; partitionId < partitionCount;
        ...
        Task* mergeTask = CreateBasicTask(jobId, mergeTaskId, MERGE_TASK, "<merge>");
        ...
        foreach (mapTaskCell, mapTaskList) {
            ...
            ShardPlacement* mapTaskPlacement =
                static_cast<ShardPlacement*>(linitial(mapTaskPlacementList));
            ...
            Task* mapOutputFetchTask = CreateBasicTask(
                jobId, taskIdIndex, MAP_OUTPUT_FETCH_TASK, fetchQueryString);
...
static List* AssignTaskList(List* sqlTaskList)
{
    ...
    foreach (sqlTaskCell, sqlTaskList) {
        ...
        foreach (mergeTaskCell, mergeTaskList) {
            Task* mergeTask = static_cast<Task*>(lfirst(mergeTaskCell));
            if (mergeTask->assignmentConstrained) {
                findPrimary = false;
                break;
            }
        }
        ...
        if (findPrimary) {
            foreach (mergeTaskCell, mergeTaskList) {
                Task* mergeTask = static_cast<Task*>(lfirst(mergeTaskCell));
                mergeTask->assignmentConstrained = true;
            }
...
bool TasksEqual(const Task* a, const Task* b)
{
...
bool TaskListMember(const List* taskList, const Task* task)
{
 ...
 * of the input lists.
 */
List* TaskListDifference(const List* list1, const List* list2)
{
    const ListCell* taskCell = NULL;
    List* resultList = NIL;
    ...
    foreach (taskCell, list1) {
        if (!TaskListMember(list2, static_cast<Task*>(lfirst(taskCell)))) {
 ...
 */
List* AssignAnchorShardTaskList(List* taskList)
{
    List* assignedTaskList = NIL;

    /* choose task assignment policy based on config value */
    if (Session_ctx::Vars().TaskAssignmentPolicy == TASK_ASSIGNMENT_GREEDY) {
        assignedTaskList = GreedyAssignTaskList(taskList);
    } else if (Session_ctx::Vars().TaskAssignmentPolicy ==
               TASK_ASSIGNMENT_FIRST_REPLICA) {
        assignedTaskList = FirstReplicaAssignTaskList(taskList);
    } else if (Session_ctx::Vars().TaskAssignmentPolicy == TASK_ASSIGNMENT_ROUND_ROBIN) {
...
static List* GreedyAssignTaskList(List* taskList)
{
    ...
    while (assignedTaskCount < taskCount) {
        ...
        foreach (workerNodeCell, workerNodeList) {
            ...
            Task* assignedTask =
                GreedyAssignTask(workerNode, taskList, activeShardPlacementLists);
                              ...
                              List* activeShardPlacementLists)
{
    ...
    uint32 replicaCount = Session_ctx::Vars().ShardReplicationFactor;
    ...
    while ((assignedTask == NULL) && (replicaIndex < replicaCount)) {
        ...
        forboth(taskCell, taskList, placementListCell, activeShardPlacementLists)
        {
            ...
            ShardPlacement* placement =
                (ShardPlacement*)list_nth(placementList, replicaIndex);
    ...
    if (assignedTask != NULL) {
        ...
        ereport(DEBUG3, (errmsg("assigned task %u to node %s:%u", assignedTask->taskId,
                                primaryPlacement->nodeName, primaryPlacement->nodePort)));
    }
 ...
 * guarantees).
 */
List* FirstReplicaAssignTaskList(List* taskList)
{
 ...
 */
List* RoundRobinAssignTaskList(List* taskList)
{
 ...
 */
List* RoundRobinReorder(List* placementList)
{
    ...
    forboth(taskCell, taskList, placementListCell, activeShardPlacementLists)
    {
        ...
        if (activePlacementCount > 0) {
            ...
            ShardPlacement* primaryPlacement =
                (ShardPlacement*)linitial(task->taskPlacementList);
            ereport(DEBUG3,
                    (errmsg("assigned task %u to node %s:%u", task->taskId,
                            primaryPlacement->nodeName, primaryPlacement->nodePort)));
...
static List* ActiveShardPlacementLists(List* taskList)
{
    ...
    foreach (taskCell, taskList) {
        ...
        activeShardPlacementList =
            SortList(activeShardPlacementList, CompareShardPlacements);
 ...
 */
int CompareShardPlacements(const void* leftElement, const void* rightElement)
{
 ...
 */
int CompareGroupShardPlacements(const void* leftElement, const void* rightElement)
{
...
static List* AssignDualHashTaskList(List* taskList)
{
    ...
    foreach (taskCell, taskList) {
        ...
        for (uint32 replicaIndex = 0;
             replicaIndex < Session_ctx::Vars().ShardReplicationFactor; replicaIndex++) {
            uint32 assignmentOffset =
                beginningNodeIndex + assignedTaskIndex + replicaIndex;
            uint32 assignmentIndex = assignmentOffset % workerNodeCount;
            WorkerNode* workerNode =
                static_cast<WorkerNode*>(list_nth(workerNodeList, assignmentIndex));
 ...
 */
void SetPlacementNodeMetadata(ShardPlacement* placement, WorkerNode* workerNode)
{
 ...
 */
int CompareTasksByTaskId(const void* leftElement, const void* rightElement)
{
    const Task* leftTask = *((const Task**)leftElement);
    const Task* rightTask = *((const Task**)rightElement);
...
static bool QueryTreeHasImproperForDeparseNodes(Node* inputNode, void* context)
{
    if (inputNode == NULL) {
        ...
        return query_tree_walker((Query*)inputNode,
                                 walker_cast0(QueryTreeHasImproperForDeparseNodes), NULL,
                                 0);
    }

    return expression_tree_walker(
        inputNode, walker_cast0(QueryTreeHasImproperForDeparseNodes), NULL);
}
...
static bool IsImproperForDeparseCoerceViaIONode(Node* inputNode)
{
    return (IsA(inputNode, CoerceViaIO) && IsA(((CoerceViaIO*)inputNode)->arg, Const) &&
