...
#ifdef DISABLE_OG_COMMENTS
typedef union {
    FunctionCallInfoData fcinfo; /* ensure enough space for nargs args is available */
...
static void AddSAOPartitionKeyRestrictionToInstance(
    ClauseWalkerContext* context, ScalarArrayOpExpr* arrayOperatorExpression);
static bool SAORestrictions(ScalarArrayOpExpr* arrayOperatorExpression,
                            Var* partitionColumn, List** requestedRestrictions);
...
static int PerformValueCompare(FunctionCallInfo compareFunctionCall, Datum a, Datum b);
...
static List* ExhaustivePrune(CitusTableCacheEntry* cacheEntry,
                             ClauseWalkerContext* context, PruningInstance* prune);
static bool ExhaustivePruneOne(ShardInterval* curInterval, ClauseWalkerContext* context,
                               PruningInstance* prune);
static int UpperShardBoundary(Datum partitionColumnValue,
                              ShardInterval** shardIntervalCache, int shardCount,
                              FunctionCallInfo compareFunction, bool includeMin);
static int LowerShardBoundary(Datum partitionColumnValue,
                              ShardInterval** shardIntervalCache, int shardCount,
                              FunctionCallInfo compareFunction, bool includeMax);
 ...
 */
List* PruneShards(Oid relationId, Index rangeTableId, List* whereClauseList,
                  Const** partitionValueConst)
{
    ...
    context.currentPruningInstance =
        static_cast<PruningInstance*>(palloc0(sizeof(PruningInstance)));

    if (cacheEntry->shardIntervalCompareFunction) {
        /* initiate function call info once (allows comparators to cache metadata) */
        InitFunctionCallInfoData(*(FunctionCallInfo)&context.compareIntervalFunctionCall,
                                 cacheEntry->shardIntervalCompareFunction, 2,
                                 cacheEntry->partitionColumn->varcollid, NULL, NULL);
    } else {
    ...
    if (cacheEntry->shardColumnCompareFunction) {
        /* initiate function call info once (allows comparators to cache metadata) */
        InitFunctionCallInfoData(*(FunctionCallInfo)&context.compareValueFunctionCall,
                                 cacheEntry->shardColumnCompareFunction, 2,
                                 cacheEntry->partitionColumn->varcollid, NULL, NULL);
    } else {
        ereport(ERROR, (errmsg("shard pruning not possible without "
    ...
    if (IsLoggableLevel(DEBUG3)) {
        ...
        if (foundRestriction && debugLoggedPruningInstances != NIL) {
            ...
            ereport(DEBUG3,
                    (errmsg("no shard pruning constraints on %s found", relationName)));
        }
    ...
    if (partitionValueConst != NULL) {
        if (singlePartitionValueConst != NULL) {
            *partitionValueConst =
                static_cast<Const*>(copyObject(singlePartitionValueConst));
        } else {
...
static bool BuildPruningTree(Node* node, PruningTreeBuildContext* context)
{
    ...
    if (IsA(node, List)) {
        return expression_tree_walker(node, walker_cast0(BuildPruningTree), context);
        ...
        if (boolExpr->boolop == NOT_EXPR) {
            ...
            context->current->childBooleanNodes =
                lappend(context->current->childBooleanNodes, child);
            ...
            return expression_tree_walker((Node*)boolExpr->args,
                                          walker_cast0(BuildPruningTree), &newContext);
        } else {
            return expression_tree_walker(node, walker_cast0(BuildPruningTree), context);
        }
    } else if (IsValidConditionNode(node, context->partitionColumn)) {
        context->current->validConstraints =
            lappend(context->current->validConstraints, node);
...
static void SimplifyPruningTree(PruningTreeNode* node, PruningTreeNode* parent)
{
    ...
    if (ConstraintCount(node) <= 1) {
        ...
        parent->validConstraints =
            list_concat(parent->validConstraints, node->validConstraints);
        parent->hasInvalidConstraints =
            parent->hasInvalidConstraints || node->hasInvalidConstraints;

        /* Remove current node from parent. Its constraint was assigned to the parent
         * above */
 ...
 */
bool ContainsFalseClause(List* whereClauseList)
{
 ...
 */
static void PrunableExpressions(PruningTreeNode* tree, ClauseWalkerContext* context)
{
    /*
...
/*
 * VarConstOpExprClause check whether an expression is a valid comparison of a Var to a
 * Const. Also obtaining the var with constant when valid.
 */
...
static bool SAORestrictions(ScalarArrayOpExpr* arrayOperatorExpression,
                            Var* partitionColumn, List** requestedRestrictions)
{
    Node* leftOpExpression = static_cast<Node*>(linitial(arrayOperatorExpression->args));
    Node* strippedLeftOpExpression = strip_implicit_coercions(leftOpExpression);
    bool usingEqualityOperator =
        OperatorImplementsEquality(arrayOperatorExpression->opno);
    ...
    if (usingEqualityOperator && strippedLeftOpExpression != NULL &&
        equal(strippedLeftOpExpression, partitionColumn) && IsA(arrayArgument, Const)) {
        ...
        ArrayIterator arrayIterator = array_create_iterator(array, 0);
        while (array_iterate(arrayIterator, &arrayElement, &isNull)) {
            ...
            if (requestedRestrictions) {
                Const* constElement = makeConst(elementType, -1, arrayConst->constcollid,
                                                typlen, arrayElement, isNull, typbyval);

                /* build partcol = arrayelem operator */
                OpExpr* arrayEqualityOp = SAORestrictionArrayEqualityOp(
                    arrayOperatorExpression, partitionColumn);
                arrayEqualityOp->args =
                    list_make2(strippedLeftOpExpression, constElement);
...
{
    PendingPruningInstance* instance =
        static_cast<PendingPruningInstance*>(palloc0(sizeof(PendingPruningInstance)));
 ...
 */
static void AddPartitionKeyRestrictionToInstance(ClauseWalkerContext* context,
                                                 OpExpr* opClause, Var* partitionColumn,
                                                 Const* constantClause)
{
    PruningInstance* prune = context->currentPruningInstance;
    ListCell* btreeInterpretationCell = NULL;
    ...
    if (constantClause->consttype != partitionColumn->vartype) {
        ...
        constantClause =
            TransformPartitionRestrictionValue(partitionColumn, constantClause, true);
    ...
    foreach (btreeInterpretationCell, btreeInterpretationList) {
        ...
        switch (btreeInterpretation->strategy) {
            case BTLessStrategyNumber: {
                if (!prune->lessConsts ||
                    PerformValueCompare(
                        (FunctionCallInfo)&context->compareValueFunctionCall,
                        constantClause->constvalue, prune->lessConsts->constvalue) < 0) {
                                ...
                                Oid secondCollId)
{
    ...
    ereport(ERROR, (errmsg("Cannot coerce %s to %s", DatumGetCString(secondTypename),
                           DatumGetCString(firstTypename))));
}

/*
 * IsValidHashRestriction checks whether an operator clause is a valid restriction for
 * hashed column.
 */
static bool IsValidHashRestriction(OpExpr* opClause)
{
...
static PruningInstance* CopyPartialPruningInstance(PruningInstance* sourceInstance)
{
    PruningInstance* newInstance =
        static_cast<PruningInstance*>(palloc(sizeof(PruningInstance)));
...
static List* ShardArrayToList(ShardInterval** shardArray, int length)
{
    ...
    for (int shardIndex = 0; shardIndex < length; shardIndex++) {
        ShardInterval* shardInterval = shardArray[shardIndex];
                      ...
                      PruningInstance* prune)
{
     ...
     */
    if (prune->equalConsts && !cacheEntry->hasOverlappingShardInterval) {
    ...
    if (prune->hashedEqualConsts) {
        ...
        int shardIndex =
            FindShardIntervalIndex(prune->hashedEqualConsts->constvalue, cacheEntry);

        if (shardIndex == INVALID_SHARD_INDEX) {
            return NIL;
        } else if (shardInterval && sortedShardIntervalArray[shardIndex]->shardId !=
    ...
    if (!cacheEntry->hasOverlappingShardInterval &&
        (prune->greaterConsts || prune->greaterEqualConsts || prune->lessConsts ||
         prune->lessEqualConsts)) {
                              ...
                              FunctionCallInfo compareFunction, bool includeMax)
{
    ...
    while (lowerBoundIndex < upperBoundIndex) {
        ...
        if ((maxValueComparison == 0 && !includeMax) || maxValueComparison > 0) {
                              ...
                              FunctionCallInfo compareFunction, bool includeMin)
{
    ...
    while (lowerBoundIndex < upperBoundIndex) {
        ...
        /* and evaluate results */
        if ((minValueComparison == 0 && !includeMin) || minValueComparison < 0) {
            /* value smaller than entire range */
            upperBoundIndex = middleIndex;
            continue;
        }
                                 ...
                                 ClauseWalkerContext* context, PruningInstance* prune)
{
    ...
    FunctionCallInfo compareFunctionCall =
        (FunctionCallInfo)&context->compareIntervalFunctionCall;
    ...
    if (prune->greaterConsts) {
         ...
         */
        if (!hasLowerBound ||
            PerformValueCompare(compareFunctionCall, prune->greaterConsts->constvalue,
                                lowerBound) >= 0) {
    ...
    if (prune->lessConsts) {
         ...
         */
        if (!hasUpperBound ||
            PerformValueCompare(compareFunctionCall, prune->lessConsts->constvalue,
                                upperBound) <= 0) {
    ...
    if (hasLowerBound) {
        lowerBoundIdx =
            LowerShardBoundary(lowerBound, sortedShardIntervalArray, shardCount,
                               compareFunctionCall, lowerBoundInclusive);
    } else {
        lowerBoundIdx = 0;
    }
    ...
    if (hasUpperBound) {
        upperBoundIdx =
            UpperShardBoundary(upperBound, sortedShardIntervalArray, shardCount,
                               compareFunctionCall, upperBoundInclusive);
    } else {
    ...
    for (int curIdx = lowerBoundIdx; curIdx <= upperBoundIdx; curIdx++) {
        remainingShardList =
            lappend(remainingShardList, sortedShardIntervalArray[curIdx]);
    }
                               ...
                               PruningInstance* prune)
{
    ...
    if (prune->equalConsts) {
        ...
        if (PerformValueCompare(compareFunctionCall, compareWith, curInterval->minValue) <
            0) {
            return true;
        }

        if (PerformValueCompare(compareFunctionCall, compareWith, curInterval->maxValue) >
            0) {
            return true;
        }
    }
    if (prune->greaterEqualConsts) {
        ...
        if (PerformValueCompare(compareFunctionCall, curInterval->maxValue, compareWith) <
            0) {
            return true;
        }
    }
    if (prune->greaterConsts) {
        ...
        if (PerformValueCompare(compareFunctionCall, curInterval->maxValue,
                                compareWith) <= 0) {
            return true;
        }
    }
    if (prune->lessEqualConsts) {
        ...
        if (PerformValueCompare(compareFunctionCall, curInterval->minValue, compareWith) >
            0) {
            return true;
        }
    }
    if (prune->lessConsts) {
        ...
        if (PerformValueCompare(compareFunctionCall, curInterval->minValue,
                                compareWith) >= 0) {
            return true;
        }
    }
...
static PruningTreeNode* CreatePruningNode(BoolExprType boolop)
{
    PruningTreeNode* node =
        static_cast<PruningTreeNode*>(palloc0(sizeof(PruningTreeNode)));
    node->boolop = boolop;
                                             ...
                                             Var* partitionColumn)
{
    ...
    arrayEqualityOp->opresulttype = get_func_rettype(arrayOperatorExpression->opfuncid);
...
static void DebugLogPruningInstance(PruningInstance* pruning, List* deparseCtx)
{
    DebugLogNode("constraint value: %s", (Node*)pruning->equalConsts, deparseCtx);
    DebugLogNode("constraint (lt) value: %s", (Node*)pruning->lessConsts, deparseCtx);
    DebugLogNode("constraint (lteq) value: %s", (Node*)pruning->lessEqualConsts,
                 deparseCtx);
    DebugLogNode("constraint (gt) value: %s", (Node*)pruning->greaterConsts, deparseCtx);
...
static int ConstraintCount(PruningTreeNode* node)
{
    return list_length(node->childBooleanNodes) + list_length(node->validConstraints) +
