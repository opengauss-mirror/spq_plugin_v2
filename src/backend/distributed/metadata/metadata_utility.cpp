...
const char* SPQ_NS_NAME = "__$spq$__";
static const char* SPQ_INTERNAL_NS_NAME = "__$spq_internal$__";
...
PG_FUNCTION_INFO_V1(spq_local_disk_space_stats);
PG_FUNCTION_INFO_V1(spq_table_size);
PG_FUNCTION_INFO_V1(spq_total_relation_size);
PG_FUNCTION_INFO_V1(spq_relation_size);
PG_FUNCTION_INFO_V1(spq_shard_sizes);

extern "C" Datum spq_local_disk_space_stats(PG_FUNCTION_ARGS);
extern "C" Datum spq_table_size(PG_FUNCTION_ARGS);
extern "C" Datum spq_total_relation_size(PG_FUNCTION_ARGS);
extern "C" Datum spq_relation_size(PG_FUNCTION_ARGS);
extern "C" Datum spq_shard_sizes(PG_FUNCTION_ARGS);

/*
 * CreateDiskSpaceTuple creates a tuple that is used as the return value
 * for spq_local_disk_space_stats.
 */
static HeapTuple CreateDiskSpaceTuple(TupleDesc tupleDescriptor, uint64 availableBytes,
                                      uint64 totalBytes)
{
...
/*
 * spq_local_disk_space_stats returns total disk space and available disk
 * space for the disk that contains PGDATA.
 */
Datum spq_local_disk_space_stats(PG_FUNCTION_ARGS)
{
    ...
    TypeFuncClass resultTypeClass = get_call_result_type(fcinfo, NULL, &tupleDescriptor);
    if (resultTypeClass != TYPEFUNC_COMPOSITE) {
        ereport(ERROR, (errmsg("return type must be a row type")));
    }

    HeapTuple diskSpaceTuple =
        CreateDiskSpaceTuple(tupleDescriptor, availableBytes, totalBytes);
...
static bool GetLocalDiskSpaceStats(uint64* availableBytes, uint64* totalBytes)
{
    ...
    if (statvfs(t_thrd.proc_cxt.DataDir, &buffer) != 0) {
        return false;
    }
 ...
 */
bool GetNodeDiskSpaceStatsForConnection(MultiConnection* connection,
                                        uint64* availableBytes, uint64* totalBytes)
{
    PGresult* result = NULL;

    char* sizeQuery = "SELECT available_disk_size, total_disk_size "
                      "FROM pg_catalog.spq_local_disk_space_stats()";
...
/*
 * spq_shard_sizes returns all shard ids and their sizes.
 */
Datum spq_shard_sizes(PG_FUNCTION_ARGS)
{
...
/*
 * spq_total_relation_size accepts a table name and returns a distributed table
 * and its indexes' total relation size.
 */
Datum spq_total_relation_size(PG_FUNCTION_ARGS)
{
    CheckCitusVersion(ERROR);
...
/*
 * spq_table_size accepts a table name and returns a distributed table's total
 * relation size.
 */
Datum spq_table_size(PG_FUNCTION_ARGS)
{
    CheckCitusVersion(ERROR);
...
/*
 * spq_relation_size accept a table name and returns a relation's 'main'
 * fork's size.
 */
Datum spq_relation_size(PG_FUNCTION_ARGS)
{
    CheckCitusVersion(ERROR);
 ...
 */
List* SendShardStatisticsQueriesInParallel(List* citusTableIds,
                                           bool useDistributedTransaction)
{
...
static List* GenerateShardStatisticsQueryList(List* workerNodeList, List* citusTableIds)
{
    ...
    foreach_declared_ptr(workerNode, workerNodeList)
    {
        ...
        shardStatisticsQueryList =
            lappend(shardStatisticsQueryList, shardStatisticsQuery);
    }
                                 ...
                                 bool failOnError, uint64* tableSize)
{
    ...
    if (Session_ctx::Trans().XactModificationLevel == XACT_MODIFICATION_DATA) {
                                         ...
                                         uint64* tableSize)
{
    ...
    StringInfo tableSizeQuery = GenerateSizeQueryOnMultiplePlacements(
        shardIntervalsOnNode, sizeQueryType, optimizePartitionCalculations);

    MultiConnection* connection =
        GetNodeConnection(connectionFlag, workerNodeName, workerNodePort);
    int queryResult =
        ExecuteOptionalRemoteCommand(connection, tableSizeQuery->data, &result);
        ...
        ereport(logLevel,
                (errcode(ERRCODE_CONNECTION_FAILURE),
                 errmsg("could not connect to %s:%d to get size of "
                        "table \"%s\"",
                        workerNodeName, workerNodePort, get_rel_name(relationId))));
        ...
        ereport(logLevel,
                (errcode(ERRCODE_CONNECTION_FAILURE),
                 errmsg("cannot parse size of table \"%s\" from %s:%d",
                        get_rel_name(relationId), workerNodeName, workerNodePort)));
    ...
    if (strlen(tableSizeString) > 0) {
        *tableSize = SafeStringToUint64(tableSizeString);
        ...
        /*
         * This means the shard is moved or dropped while spq_total_relation_size is
 ...
 * on the group.
 */
List* GroupShardPlacementsForTableOnGroup(Oid relationId, int32 groupId)
{
    ...
    for (int shardIndex = 0; shardIndex < shardIntervalArrayLength; shardIndex++) {
        ...
        for (int placementIndex = 0; placementIndex < numberOfPlacements;
            ...
            if (placementArray[placementIndex].groupId == groupId) {
                GroupShardPlacement* placement = static_cast<GroupShardPlacement*>(
                    palloc0(sizeof(GroupShardPlacement)));
 ...
 */
StringInfo GenerateSizeQueryOnMultiplePlacements(List* shardIntervalList,
                                                 ...
                                                 bool optimizePartitionCalculations)
{
    ...
    foreach_declared_ptr(shardInterval, shardIntervalList)
    {
        ...
        if (optimizePartitionCalculations &&
            PartitionedTable(shardInterval->relationId)) {
    ...
    char* subqueryForPartitionedShards = GenerateSizeQueryForRelationNameList(
        partitionedShardNames,
        GetWorkerPartitionedSizeUDFNameBySizeQueryType(sizeQueryType));

    /* SELECT SUM(pg_..._size) FROM VALUES (...) */
    char* subqueryForNonPartitionedShards = GenerateSizeQueryForRelationNameList(
        nonPartitionedShardNames, GetSizeQueryBySizeQueryType(sizeQueryType));

    appendStringInfo(selectQuery, "SELECT (%s) + (%s);", subqueryForPartitionedShards,
                     subqueryForNonPartitionedShards);
 ...
 * SELECT SUM( <sizeFunction>(relid) ) FROM (VALUES (<shardName>), (<shardName>), ...) as
 * q(relid)
 */
static char* GenerateSizeQueryForRelationNameList(List* quotedShardNames,
                                                  char* sizeFunction)
                                                    ...
                                                    List* citusTableIds)
{
    ...
    foreach_declared_oid(relationId, citusTableIds)
    {
        ...
        Relation relation = try_relation_open(relationId, AccessShareLock);
        if (relation != NULL) {
            List* shardIntervalsOnNode =
                ShardIntervalsOnWorkerGroup(workerNode, relationId);
            if (list_length(shardIntervalsOnNode) == 0) {
                relation_close(relation, AccessShareLock);
                continue;
            }
            char* shardIdNameValues = GenerateShardIdNameValuesForShardList(
                shardIntervalsOnNode, !insertedValues);
            insertedValues = true;
    ...
    appendStringInfoString(allShardStatisticsQuery, ") t(shard_id, table_name) WHERE ");
    appendStringInfoString(allShardStatisticsQuery, SPQ_INTERNAL_NS_NAME);
    appendStringInfoString(allShardStatisticsQuery,
 ...
 */
int CompareShardPlacementsByWorker(const void* leftElement, const void* rightElement)
{
    ...
    int nodeNameCmp =
        strncmp(leftPlacement->nodeName, rightPlacement->nodeName, WORKER_LENGTH);
    if (nodeNameCmp != 0) {
        return nodeNameCmp;
 ...
 * group id.
 */
int CompareShardPlacementsByGroupId(const void* leftElement, const void* rightElement)
{
    const ShardPlacement* leftPlacement = *((const ShardPlacement**)leftElement);
    const ShardPlacement* rightPlacement = *((const ShardPlacement**)rightElement);
 ...
 */
uint32 TableShardReplicationFactor(Oid relationId)
{
        ...
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                 errmsg("cannot find the replication factor of the "
                        "table %s",
                        relationName),
                 errdetail("The table %s does not have any shards.", relationName)));
    }
 ...
 */
List* LoadShardIntervalList(Oid relationId)
{
 ...
 * directly.
 */
List* LoadUnsortedShardIntervalListViaCatalog(Oid relationId)
{
    List* shardIntervalList = NIL;
    ...
    {
        ShardInterval* interval = TupleToShardInterval(distShardTuple, distShardTupleDesc,
 ...
 */
ShardInterval* LoadShardIntervalWithLongestShardName(Oid relationId)
{
    CitusTableCacheEntry* cacheEntry = GetCitusTableCacheEntry(relationId);
 ...
 */
int ShardIntervalCount(Oid relationId)
{
    CitusTableCacheEntry* cacheEntry = GetCitusTableCacheEntry(relationId);
 ...
 */
List* LoadShardList(Oid relationId)
{
    CitusTableCacheEntry* cacheEntry = GetCitusTableCacheEntry(relationId);
    List* shardList = NIL;
 ...
 */
ShardInterval* CopyShardInterval(ShardInterval* srcInterval)
{
    ShardInterval* destInterval =
        static_cast<ShardInterval*>(palloc0(sizeof(ShardInterval)));
    ...
    if (destInterval->minValueExists) {
        destInterval->minValue = datumCopy(srcInterval->minValue, srcInterval->valueByVal,
                                           srcInterval->valueTypeLen);
    }

    destInterval->maxValue = 0;
    if (destInterval->maxValueExists) {
        destInterval->maxValue = datumCopy(srcInterval->maxValue, srcInterval->valueByVal,
                                           srcInterval->valueTypeLen);
    }
 ...
 */
uint64 ShardLength(uint64 shardId)
{
 ...
 */
bool NodeGroupHasShardPlacements(int32 groupId)
{
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_placement_groupid, BTEqualStrategyNumber,
                F_INT4EQ, Int32GetDatum(groupId));

    SysScanDesc scanDescriptor = systable_beginscan(
        pgPlacement, DistPlacementGroupidIndexId(), indexOK, NULL, scanKeyCount, scanKey);
 ...
 */
bool IsActiveShardPlacement(ShardPlacement* shardPlacement)
{
 ...
 * node.
 */
bool IsRemoteShardPlacement(ShardPlacement* shardPlacement)
{
 ...
 * workenode.
 */
bool IsPlacementOnWorkerNode(ShardPlacement* placement, WorkerNode* workerNode)
{
 ...
 */
List* FilterShardPlacementList(List* shardPlacementList, bool (*filter)(ShardPlacement*))
{
    ...
    foreach_declared_ptr(shardPlacement, shardPlacementList)
    {
        if (filter(shardPlacement)) {
            filteredShardPlacementList =
                lappend(filteredShardPlacementList, shardPlacement);
        }
    }
...
/*
 * FilterActiveShardPlacementListByNode filters a list of active shard placements based on
 * given nodeName and nodePort.
 */
List* FilterActiveShardPlacementListByNode(List* shardPlacementList,
                                           WorkerNode* workerNode)
{
 ...
 */
List* ActiveShardPlacementListOnGroup(uint64 shardId, int32 groupId)
{
    ...
    foreach_declared_ptr(shardPlacement, activePlacementList)
    {
        if (shardPlacement->groupId == groupId) {
            activeShardPlacementListOnGroup =
                lappend(activeShardPlacementListOnGroup, shardPlacement);
        }
    }
 ...
 */
List* ActiveShardPlacementList(uint64 shardId)
{
    List* shardPlacementList = ShardPlacementList(shardId);

    List* activePlacementList =
        FilterShardPlacementList(shardPlacementList, IsActiveShardPlacement);
 ...
 */
List* ShardPlacementListSortedByWorker(uint64 shardId)
{
    List* shardPlacementList = ShardPlacementList(shardId);
 ...
 */
ShardPlacement* ActiveShardPlacement(uint64 shardId, bool missingOk)
{
    List* activePlacementList = ActiveShardPlacementList(shardId);
    ShardPlacement* shardPlacement = NULL;
        ...
        if (!missingOk) {
            ereport(
                ...
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("could not find any healthy placement for shard " UINT64_FORMAT,
                        shardId)));
        }
 ...
 * a shard.
 */
WorkerNode* ActiveShardPlacementWorkerNode(uint64 shardId)
{
    ...
    ShardPlacement* sourceShardPlacement =
        static_cast<ShardPlacement*>(linitial(sourcePlacementList));
    WorkerNode* sourceShardToCopyNode =
        FindNodeWithNodeId(sourceShardPlacement->nodeId, missingOK);
 ...
 */
List* BuildShardPlacementList(int64 shardId)
{
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_placement_shardid, BTEqualStrategyNumber,
                F_INT8EQ, Int64GetDatum(shardId));

    SysScanDesc scanDescriptor = systable_beginscan(
        pgPlacement, DistPlacementShardidIndexId(), indexOK, NULL, scanKeyCount, scanKey);
 ...
 * representation, and returns the converted shard placements in a new list.
 */
List* AllShardPlacementsOnNodeGroup(int32 groupId)
{
    List* shardPlacementList = NIL;
    ScanKeyData scanKey[1];
    int scanKeyCount = 1;
 ...
 */
static GroupShardPlacement* TupleToGroupShardPlacement(TupleDesc tupleDescriptor,
                                                       HeapTuple heapTuple)
{
    bool isNullArray[Natts_pg_dist_placement];
    Datum datumArray[Natts_pg_dist_placement];

    if (HeapTupleHeaderGetNatts(heapTuple->t_data, nullptr) != Natts_pg_dist_placement ||
    ...
    shardPlacement->placementId =
        DatumGetInt64(datumArray[Anum_pg_dist_placement_placementid - 1]);
    shardPlacement->shardId =
        DatumGetInt64(datumArray[Anum_pg_dist_placement_shardid - 1]);
    shardPlacement->shardLength =
        DatumGetInt64(datumArray[Anum_pg_dist_placement_shardlength - 1]);
    shardPlacement->groupId =
        DatumGetInt32(datumArray[Anum_pg_dist_placement_groupid - 1]);

    return shardPlacement;
}

/*
 * LookupTaskPlacementHostAndPort sets the nodename and nodeport for the given task
 * placement with a lookup.
 */
void LookupTaskPlacementHostAndPort(ShardPlacement* taskPlacement, char** nodeName,
                                    int* nodePort)
{
 ...
 */
bool IsDummyPlacement(ShardPlacement* taskPlacement)
{
 ...
 */
void InsertShardRow(Oid relationId, uint64 shardId, char storageType, text* shardMinValue,
                    text* shardMaxValue)
{
 ...
 */
ShardPlacement* InsertShardPlacementRowGlobally(uint64 shardId, uint64 placementId,
 ...
 */
uint64 InsertShardPlacementRow(uint64 shardId, uint64 placementId, uint64 shardLength,
                               int32 groupId)
{
 ...
 */
void InsertIntoPgDistPartition(Oid relationId, char distributionMethod,
                               ...
                               char replicationModel, bool autoConverted)
{
    ...
    newValues[Anum_pg_dist_partition_logicalrelid - 1] = ObjectIdGetDatum(relationId);
    newValues[Anum_pg_dist_partition_partmethod - 1] = CharGetDatum(distributionMethod);
    ...
    HeapTuple newTuple =
        heap_form_tuple(RelationGetDescr(pgDistPartition), newValues, newNulls);
...
static void RecordDistributedRelationDependencies(Oid distributedRelationId)
{
    ...
    citusExtensionAddr.objectId = get_extension_oid(SPQ_EXTENSION_NAME, false);
 ...
 * metadata cache.
 */
void DeletePartitionRow(Oid distributedRelationId)
{
    ScanKeyData scanKey[1];
    int scanKeyCount = 1;
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_partition_logicalrelid, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(distributedRelationId));
 ...
 */
void DeleteShardRow(uint64 shardId)
{
    ScanKeyData scanKey[1];
    int scanKeyCount = 1;
    bool indexOK = true;
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_shard_shardid, BTEqualStrategyNumber, F_INT8EQ,
                Int64GetDatum(shardId));

    SysScanDesc scanDescriptor = systable_beginscan(
        pgDistShard, DistShardShardidIndexId(), indexOK, NULL, scanKeyCount, scanKey);

    HeapTuple heapTuple = systable_getnext(scanDescriptor);
    if (!HeapTupleIsValid(heapTuple)) {
        ereport(ERROR,
                (errmsg("could not find valid entry for shard " UINT64_FORMAT, shardId)));
    }
 ...
 */
void DeleteShardPlacementRowGlobally(uint64 placementId)
{
    DeleteShardPlacementRow(placementId);

    char* deletePlacementCommand = DeletePlacementMetadataCommand(placementId);
 ...
 */
void DeleteShardPlacementRow(uint64 placementId)
{
    const int scanKeyCount = 1;
    ScanKeyData scanKey[1];
    bool indexOK = true;
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_placement_placementid, BTEqualStrategyNumber,
                F_INT8EQ, Int64GetDatum(placementId));

    SysScanDesc scanDescriptor =
        systable_beginscan(pgDistPlacement, DistPlacementPlacementidIndexId(), indexOK,
                           NULL, scanKeyCount, scanKey);

    HeapTuple heapTuple = systable_getnext(scanDescriptor);
    if (heapTuple == NULL) {
        ereport(ERROR,
                (errmsg("could not find valid entry for shard placement " INT64_FORMAT,
                        placementId)));
    }

    uint64 shardId =
        heap_getattr(heapTuple, Anum_pg_dist_placement_shardid, tupleDescriptor, &isNull);
    if (HeapTupleHeaderGetNatts(heapTuple->t_data, nullptr) != Natts_pg_dist_placement ||
        HeapTupleHasNulls(heapTuple)) {
        ereport(ERROR, (errmsg("unexpected null in pg_dist_placement tuple")));
    }
 ...
 * by placementId.
 */
void UpdatePlacementGroupId(uint64 placementId, int groupId)
{
    ScanKeyData scanKey[1];
    int scanKeyCount = 1;
    bool indexOK = true;
    ...
    if (!HeapTupleIsValid(heapTuple)) {
        ereport(ERROR,
                (errmsg("could not find valid entry for shard placement " UINT64_FORMAT,
                        placementId)));
    }
    ...
    uint64 shardId = DatumGetInt64(heap_getattr(heapTuple, Anum_pg_dist_placement_shardid,
 ...
 * by citusTableId.
 */
void UpdatePgDistPartitionAutoConverted(Oid citusTableId, bool autoConverted)
{
    ScanKeyData scanKey[1];
    int scanKeyCount = 1;
    bool indexOK = true;
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_partition_logicalrelid, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(citusTableId));

    SysScanDesc scanDescriptor =
        systable_beginscan(pgDistPartition, DistPartitionLogicalRelidIndexId(), indexOK,
                           NULL, scanKeyCount, scanKey);
 ...
 */
void UpdateDistributionColumnGlobally(Oid relationId, char distributionMethod,
 ...
 */
void UpdateDistributionColumn(Oid relationId, char distributionMethod,
                              Var* distributionColumn, int colocationId)
{
    ScanKeyData scanKey[1];
    int scanKeyCount = 1;
 ...
 */
void UpdateNoneDistTableMetadataGlobally(Oid relationId, char replicationModel,
                                         uint32 colocationId, bool autoConverted)
{
    ...
    if (ShouldSyncTableMetadata(relationId)) {
        char* metadataCommand = UpdateNoneDistTableMetadataCommand(
            relationId, replicationModel, colocationId, autoConverted);
 ...
 */
void UpdateNoneDistTableMetadata(Oid relationId, char replicationModel,
                                 uint32 colocationId, bool autoConverted)
{
 ...
 */
void EnsureTablePermissions(Oid relationId, AclMode mode)
{
 ...
 */
void EnsureTableOwner(Oid relationId)
{
    if (!object_ownercheck(RelationRelationId, relationId, GetUserId())) {
        aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_CLASS, get_rel_name(relationId));
    }
}
 ...
 * not. Superusers are regarded as owners.
 */
void EnsureSchemaOwner(Oid schemaId)
{
 ...
 * treated equally.
 */
void EnsureFunctionOwner(Oid functionId)
{
    if (!object_ownercheck(ProcedureRelationId, functionId, GetUserId())) {
        aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_PROC, get_func_name(functionId));
    }
}

/*
 * EnsureHashDistributedTable error out if the given relation is not a hash distributed
 * table with the given message.
 */
void EnsureHashDistributedTable(Oid relationId)
{
 ...
 */
void EnsureHashOrSingleShardDistributedTable(Oid relationId)
{
    if (!IsCitusTableType(relationId, HASH_DISTRIBUTED) &&
...
Oid TableOwnerOid(Oid relationId)
{
 ...
 */
char* TableOwner(Oid relationId)
{
    return GetUserNameFromId(TableOwnerOid(relationId));
}
 ...
 */
bool IsForeignTable(Oid relationId)
{
...
bool HasRunnableBackgroundTask()
{
    ...
    for (int i = 0; !hasScheduledTask && i < lengthof(taskStatus); i++) {
        ...
        SysScanDesc scanDescriptor = systable_beginscan(
            pgDistBackgroundTasks, DistBackgroundTaskStatusTaskIdIndexId(), indexOK, NULL,
        ...
        while (HeapTupleIsValid(taskTuple = systable_getnext(scanDescriptor))) {
            ...
            BackgroundTask* task =
                DeformBackgroundTaskHeapTuple(tupleDescriptor, taskTuple);
 ...
 */
BackgroundJobStatus BackgroundJobStatusByOid(Oid enumOid)
{
 ...
 * based on the Oid of the SQL enum value.
 */
BackgroundTaskStatus BackgroundTaskStatusByOid(Oid enumOid)
{
 ...
 * terminal state.
 */
bool IsBackgroundJobStatusTerminal(BackgroundJobStatus status)
{
    switch (status) {
 ...
 */
bool IsBackgroundTaskStatusTerminal(BackgroundTaskStatus status)
{
    switch (status) {
 ...
 */
Oid BackgroundJobStatusOid(BackgroundJobStatus status)
{
    switch (status) {
        case BACKGROUND_JOB_STATUS_SCHEDULED: {
...
/*
 * BackgroundTaskStatusOid returns the Oid corresponding to SQL enum value corresponding
 * to the BackgroundTaskStatus.
 */
Oid BackgroundTaskStatusOid(BackgroundTaskStatus status)
{
    switch (status) {
        case BACKGROUND_TASK_STATUS_BLOCKED: {
 ...
 */
bool HasNonTerminalJobOfType(const char* jobType, int64* jobIdOut)
{
    ...
    BackgroundJobStatus jobStatus[] = {
        BACKGROUND_JOB_STATUS_RUNNING, BACKGROUND_JOB_STATUS_CANCELLING,
        BACKGROUND_JOB_STATUS_FAILING, BACKGROUND_JOB_STATUS_SCHEDULED};
    ...
    for (int i = 0; !foundJob && i < lengthof(jobStatus); i++) {
        ...
        ScanKeyInit(&scanKey[0], Anum_pg_dist_background_job_state, BTEqualStrategyNumber,
                    F_OIDEQ, ObjectIdGetDatum(BackgroundJobStatusOid(jobStatus[i])));

        /* pg_dist_background_job.job_type == jobType */
        ScanKeyInit(&scanKey[1], Anum_pg_dist_background_job_job_type,
                    BTEqualStrategyNumber, F_NAMEEQ, NameGetDatum(&jobTypeName));

        SysScanDesc scanDescriptor = systable_beginscan(
            pgDistBackgroundJob, InvalidOid, /* TODO use an actual index here */
 ...
 * created job.
 */
int64 CreateBackgroundJob(const char* jobType, const char* description)
{
    ...
    HeapTuple newTuple =
        heap_form_tuple(RelationGetDescr(pgDistBackgroundJobs), values, isnull);
 ...
 */
BackgroundTask* ScheduleBackgroundTask(int64 jobId, Oid owner, char* command,
                                       int dependingTaskCount, int64 dependingTaskIds[],
                                       int nodesInvolvedCount, int32 nodesInvolved[])
{
    ...
    /* 1. verify job exist */
    {
        ...
        SysScanDesc scanDescriptor =
            systable_beginscan(pgDistBackgroundJob, DistBackgroundJobPKeyIndexId(),
                               indexOK, NULL, lengthof(scanKey), scanKey);
    ...
    /* 2. insert new task */
    {
        ...
        HeapTuple newTuple =
            heap_form_tuple(RelationGetDescr(pgDistBackgroundTask), values, nulls);
        CatalogTupleInsert(pgDistBackgroundTask, newTuple);

        task = static_cast<BackgroundTask*>(palloc0(sizeof(BackgroundTask)));
        task->taskid = taskId;
    ...
    /* 3. insert dependencies into catalog */
    {
        for (int i = 0; i < dependingTaskCount; i++) {
            /* 3.1 after verifying the task exists for this job */
            {
                ...
                SysScanDesc scanDescriptor = systable_beginscan(
                    pgDistBackgroundTask, DistBackgroundTaskJobIdTaskIdIndexId(), indexOK,
            ...
            values[Anum_pg_dist_background_task_depend_job_id - 1] = Int64GetDatum(jobId);
 ...
 */
void ResetRunningBackgroundTasks(void)
{
    const int scanKeyCount = 1;
    ScanKeyData scanKey[1];
    const bool indexOK = true;
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_background_task_status, BTEqualStrategyNumber,
                F_OIDEQ, ObjectIdGetDatum(CitusTaskStatusRunningId()));

    SysScanDesc scanDescriptor =
        systable_beginscan(pgDistBackgroundTasks, DistBackgroundTaskStatusTaskIdIndexId(),
    ...
    while (HeapTupleIsValid(taskTuple = systable_getnext(scanDescriptor))) {
        ...
        if (!isnull[Anum_pg_dist_background_task_pid - 1]) {
            ...
            if (locked == LOCKACQUIRE_NOT_AVAIL) {
                ...
                Datum signalSuccessDatum =
                    DirectFunctionCall2(pg_terminate_backend, pidDatum, timeoutDatum);
                 ...
                 * list for later
                 */
                int64* taskIdTarget = static_cast<int64*>(palloc0(sizeof(int64)));
                *taskIdTarget = taskId;
        ...
        taskTuple =
            heap_modify_tuple(taskTuple, tupleDescriptor, values, isnull, replace);
 ...
 */
static BackgroundJob* DeformBackgroundJobHeapTuple(TupleDesc tupleDescriptor,
                                                   HeapTuple jobTuple)
{
    Datum values[Natts_pg_dist_background_job] = {0};
    ...
    BackgroundJob* job = static_cast<BackgroundJob*>(palloc0(sizeof(BackgroundJob)));
    ...
    if (!nulls[Anum_pg_dist_background_job_job_type - 1]) {
        Name jobTypeName = DatumGetName(values[Anum_pg_dist_background_job_job_type - 1]);
                                                     ...
                                                     HeapTuple taskTuple)
{
    ...
    BackgroundTask* task = static_cast<BackgroundTask*>(palloc0(sizeof(BackgroundTask)));
    ...
    task->command =
        text_to_cstring(DatumGetTextP(values[Anum_pg_dist_background_task_command - 1]));
 ...
 */
static bool BackgroundTaskHasUmnetDependencies(int64 jobId, int64 taskId)
{
    ...
    SysScanDesc scanDescriptor = systable_beginscan(
        pgDistBackgroundTasksDepend, DistBackgroundTaskDependTaskIdIndexId(), indexOK,
        NULL, lengthof(scanKey), scanKey);
 ...
 */
static bool BackgroundTaskReadyToRun(BackgroundTask* task)
{
    if (task->not_before) {
 ...
 * available.
 */
BackgroundTask* GetRunnableBackgroundTask(void)
{
    Relation pgDistBackgroundTasks =
        table_open(DistBackgroundTaskRelationId(), ExclusiveLock);
 ...
 */
BackgroundJob* GetBackgroundJobByJobId(int64 jobId)
{
    ScanKeyData scanKey[1] = {0};
    bool indexOK = true;
    ...
    /* pg_dist_background_task.job_id == $jobId */
    ScanKeyInit(&scanKey[0], Anum_pg_dist_background_job_job_id, BTEqualStrategyNumber,
                F_INT8EQ, Int64GetDatum(jobId));

    SysScanDesc scanDescriptor =
        systable_beginscan(pgDistBackgroundJobs, DistBackgroundJobPKeyIndexId(), indexOK,
                           NULL, lengthof(scanKey), scanKey);
 ...
 */
BackgroundTask* GetBackgroundTaskByTaskId(int64 taskId)
{
    ScanKeyData scanKey[1] = {0};
    bool indexOK = true;
    ...
    /* pg_dist_background_task.task_id == $taskId */
    ScanKeyInit(&scanKey[0], Anum_pg_dist_background_task_task_id, BTEqualStrategyNumber,
                F_INT8EQ, Int64GetDatum(taskId));

    SysScanDesc scanDescriptor =
        systable_beginscan(pgDistBackgroundTasks, DistBackgroundTaskPKeyIndexId(),
                           indexOK, NULL, lengthof(scanKey), scanKey);
 ...
 */
static JobTaskStatusCounts JobTasksStatusCount(int64 jobId)
{
    Relation pgDistBackgroundTasks =
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_background_task_job_id, BTEqualStrategyNumber,
                F_INT8EQ, Int64GetDatum(jobId));

    SysScanDesc scanDescriptor =
        systable_beginscan(pgDistBackgroundTasks, DistBackgroundTaskJobIdTaskIdIndexId(),
                           indexOK, NULL, lengthof(scanKey), scanKey);
    ...
    while (HeapTupleIsValid(heapTuple = systable_getnext(scanDescriptor))) {
        ...
        Oid statusOid = DatumGetObjectId(values[Anum_pg_dist_background_task_status - 1]);
 ...
 */
void UpdateBackgroundJob(int64 jobId)
{
    ...
    if (!HeapTupleIsValid(heapTuple)) {
        ereport(ERROR,
                (errmsg("could not find background jobs entry for job_id: " UINT64_FORMAT,
                        jobId)));
    }
 ...
 * catalog.
 */
void UpdateBackgroundTask(BackgroundTask* task)
{
    Relation pgDistBackgroundTasks =
        table_open(DistBackgroundTaskRelationId(), RowExclusiveLock);
    TupleDesc tupleDescriptor = RelationGetDescr(pgDistBackgroundTasks);
    ...
    ScanKeyInit(&scanKey[0], Anum_pg_dist_background_task_task_id, BTEqualStrategyNumber,
                F_INT8EQ, Int64GetDatum(task->taskid));
    ...
    if (task->pid) {
        ...
        updated |=
            SetFieldNull(Anum_pg_dist_background_task_pid, values, isnull, replace);
    }
    ...
    if (task->message) {
        ...
        updated |=
            SetFieldNull(Anum_pg_dist_background_task_message, values, isnull, replace);
    }
 ...
 */
static List* GetDependantTasks(int64 jobId, int64 taskId)
{
    ...
    SysScanDesc scanDescriptor = systable_beginscan(
        pgDistBackgroundTasksDepends, DistBackgroundTaskDependDependsOnIndexId(), indexOK,
        NULL, lengthof(scanKey), scanKey);
    ...
    while (HeapTupleIsValid(heapTuple = systable_getnext(scanDescriptor))) {
        ...
        int64* dTaskId = static_cast<int64*>(palloc0(sizeof(int64)));
 ...
 * would perform.
 */
List* CancelTasksForJob(int64 jobid)
{
    Relation pgDistBackgroundTasks =
        table_open(DistBackgroundTaskRelationId(), ExclusiveLock);
    TupleDesc tupleDescriptor = RelationGetDescr(pgDistBackgroundTasks);
    ...
    while (HeapTupleIsValid(taskTuple = systable_getnext(scanDescriptor))) {
        ...
        nulls[Anum_pg_dist_background_task_status - 1] = false;
        values[Anum_pg_dist_background_task_status - 1] =
            ObjectIdGetDatum(BackgroundTaskStatusOid(newStatus));
        replace[Anum_pg_dist_background_task_status - 1] = true;

        taskTuple = heap_modify_tuple(taskTuple, tupleDescriptor, values, nulls, replace);
        CatalogTupleUpdate(pgDistBackgroundTasks, &taskTuple->t_self, taskTuple);
    }
 ...
 */
void UnscheduleDependentTasks(BackgroundTask* task)
{
    Relation pgDistBackgroundTasks =
        table_open(DistBackgroundTaskRelationId(), RowExclusiveLock);
    TupleDesc tupleDescriptor = RelationGetDescr(pgDistBackgroundTasks);
    ...
    while (list_length(dependantTasks) > 0) {
        ...
        dependantTasks =
            list_concat(dependantTasks, GetDependantTasks(task->jobid, cTaskId));
 ...
 */
void UnblockDependingBackgroundTasks(BackgroundTask* task)
{
    Relation pgDistBackgroundTasksDepend =
        table_open(DistBackgroundTaskDependRelationId(), RowExclusiveLock);
