...
static void ShardCopyDestReceiverReceive(TupleTableSlot* slot, DestReceiver* dest);
...
static void ConnectToRemoteAndStartCopy(ShardCopyDestReceiver* copyDest)
{
    ...
    if (copyDest->useNodeId) {
        WorkerNode* workerNode =
            FindNodeWithNodeId(copyDest->destinationNodeId, false /* missingOk */);
        copyDest->connection = GetNodeUserDatabaseConnection(
            connectionFlags, workerNode->workerName, workerNode->workerPort, currentUser,
            NULL /* database (current) */);
    } else {
        copyDest->connection = GetNodeUserDatabaseConnection(
            connectionFlags, copyDest->destinationNodeName, copyDest->destinationNodePort,
            currentUser, NULL /* database (current) */);
    }
    ...
    StringInfo copyStatement = ConstructShardCopyStatement(
        copyDest->destinationShardFullyQualifiedName, copyDest->copyOutState->binary,
    ...
    PGresult* result =
        GetRemoteCommandResult(copyDest->connection, true /* raiseInterrupts */);
 ...
 */
DestReceiver* CreateShardCopyDestReceiver(EState* executorState,
                                          List* destinationShardFullyQualifiedName,
                                          uint32_t destinationNodeId)
{
    ShardCopyDestReceiver* copyDest =
        (ShardCopyDestReceiver*)palloc0(sizeof(ShardCopyDestReceiver));
                                                 ...
                                                 bool localCopy)
{
    ...
    copyDest->useNodeId = false;
    copyDest->destinationNodeName = destinationNodeName;
    copyDest->destinationNodePort = destinationNodePort;
    copyDest->destinationShardFullyQualifiedName = destinationShardFullyQualifiedName;
    copyDest->tuplesSent = 0;
    copyDest->connection = NULL;
    copyDest->useLocalCopy = localCopy;
...
static void ShardCopyDestReceiverReceive(TupleTableSlot* slot, DestReceiver* dest)
{
    ...
    if (copyDest->useLocalCopy) {
        ...
        if (copyOutState->fe_msgbuf->len >
            Session_ctx::Vars().LocalCopyFlushThresholdByte) {
        ...
        AppendCopyRowData(columnValues, columnNulls, copyDest->tupleDescriptor,
                          copyOutState, copyDest->columnOutputFunctions,
                          NULL /* columnCoercionPaths */);
        if (!PutRemoteCopyData(copyDest->connection, copyOutState->fe_msgbuf->data,
                               copyOutState->fe_msgbuf->len)) {
            char* destinationShardSchemaName = static_cast<char*>(
                linitial(copyDest->destinationShardFullyQualifiedName));
            char* destinationShardRelationName =
                static_cast<char*>(lsecond(copyDest->destinationShardFullyQualifiedName));
            ...
            if (copyDest->useNodeId) {
                ereport(ERROR, (errcode(ERRCODE_IO_ERROR),
                                errmsg("Failed to COPY to shard %s.%s : %s,",
                                       ...
                                       destinationShardRelationName, errorMessage),
                ...
                ereport(ERROR, (errcode(ERRCODE_IO_ERROR),
                                errmsg("Failed to COPY to shard %s.%s : %s,",
                                       destinationShardSchemaName,
                                       destinationShardRelationName, errorMessage),
                                errdetail("failed to send %d bytes %s on node %s:%u",
                                          copyOutState->fe_msgbuf->len,
                                          copyOutState->fe_msgbuf->data,
...
/*
 * ShardCopyDestReceiverStartup implements the rStartup interface of
 * ShardCopyDestReceiver.
 */
static void ShardCopyDestReceiverStartup(DestReceiver* dest, int operation,
                                         TupleDesc inputTupleDescriptor)
{
    ...
    copyOutState->binary = Session_ctx::Vars().EnableBinaryProtocol &&
                           CanUseBinaryCopyFormat(inputTupleDescriptor);
 ...
 * data in buffer.
 */
static void ShardCopyDestReceiverShutdown(DestReceiver* dest)
{
    ShardCopyDestReceiver* copyDest = (ShardCopyDestReceiver*)dest;
 ...
 */
static void ShardCopyDestReceiverDestroy(DestReceiver* dest)
{
    ShardCopyDestReceiver* copyDest = (ShardCopyDestReceiver*)dest;
...
/*
 *  CopyableColumnNamesFromTupleDesc function creates and returns a comma seperated column
 * names string  to be used in COPY and SELECT statements when copying a table. The COPY
 * and SELECT statements should filter out the GENERATED columns since COPY statement
 * fails to handle them. Iterating over the attributes of the table we also need to skip
 * the dropped columns.
 */
const char* CopyableColumnNamesFromTupleDesc(TupleDesc tupDesc)
{
...
/*
 *  CopyableColumnNamesFromRelationName function is a wrapper for
 * CopyableColumnNamesFromTupleDesc.
 */
const char* CopyableColumnNamesFromRelationName(const char* schemaName,
...
static StringInfo ConstructShardCopyStatement(List* destinationShardFullyQualifiedName,
                                              bool useBinaryFormat, TupleDesc tupleDesc)
{
    char* destinationShardSchemaName =
        static_cast<char*>(linitial(destinationShardFullyQualifiedName));
    char* destinationShardRelationName =
        static_cast<char*>(lsecond(destinationShardFullyQualifiedName));
    ...
    appendStringInfo(command, "COPY %s.%s (%s) FROM STDIN",
                     ...
                     quote_identifier(destinationShardRelationName), columnList);
...
static void WriteLocalTuple(TupleTableSlot* slot, ShardCopyDestReceiver* copyDest)
{
    ...
    bool shouldAddBinaryHeaders =
        (isBinaryCopy && localCopyOutState->fe_msgbuf->len == 0);
                             ...
                             CopyOutState localCopyOutState)
{
    ...
    char* destinationShardSchemaName =
        static_cast<char*>(linitial(copyDest->destinationShardFullyQualifiedName));
    char* destinationShardRelationName =
        static_cast<char*>(lsecond(copyDest->destinationShardFullyQualifiedName));

    Oid destinationSchemaOid =
        get_namespace_oid(destinationShardSchemaName, false /* missing_ok */);
    Oid destinationShardOid =
        get_relname_relid(destinationShardRelationName, destinationSchemaOid);

    DefElem* binaryFormatOption = NULL;
    if (isBinaryCopy) {
        binaryFormatOption = makeDefElem("format", (Node*)makeString("binary"));
    }

    Relation shard = table_open(destinationShardOid, RowExclusiveLock);
    ParseState* pState = make_parsestate(NULL /* parentParseState */);
#ifdef DISABLE_OG_COMMENTS
    (void)addRangeTableEntryForRelation(pState, shard, AccessShareLock, NULL /* alias */,
                                        false /* inh */, false /* inFromCl */);
#else
    (void)addRangeTableEntryForRelation(pState, shard, NULL, false /* inh */,
                                        false /* inFromCl */);
#endif
    ...
    CopyFromState cstate =
        BeginCopyFrom(pState, shard, NULL /* whereClause */, NULL /* fileName */,
                      false /* is_program */, ReadFromLocalBufferCallback,
                      NULL /* attlist (NULL is all columns) */, options);
#else
    CopyState cstate = BeginCopyFrom(shard, NULL, NULL, options, NULL, NULL,
                                     ReadFromLocalBufferCallback);
#endif
    CopyFrom(cstate);
    EndCopyFrom(cstate);
                                       ...
                                       int maxRead)
{
    ...
    if (bytesToRead > 0) {
        memcpy_s(outBuf, bytesToRead, &LocalCopyBuffer->data[LocalCopyBuffer->cursor],
                 bytesToRead);
    }
