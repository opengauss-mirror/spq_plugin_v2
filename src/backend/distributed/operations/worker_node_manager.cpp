 ...
 */
WorkerNode* WorkerGetRoundRobinCandidateNode(List* workerNodeList, uint64 shardId,
...
/*
 * ActivePrimaryNonCoordinatorNodeCount returns the number of groups with a primary in the
 * cluster. This method excludes coordinator even if it is added as a worker to cluster.
 */
uint32 ActivePrimaryNonCoordinatorNodeCount(void)
{
 ...
 */
uint32 ActiveReadableNodeCount(void)
{
 ...
 */
bool NodeIsCoordinator(WorkerNode* node)
{
 ...
 */
static List* FilterActiveNodeListFunc(LOCKMODE lockMode,
                                      bool (*checkFunction)(WorkerNode*))
{
    ...
    while ((workerNode = static_cast<WorkerNode*>(hash_seq_search(&status))) != NULL) {
        if (workerNode->isActive && checkFunction(workerNode)) {
            WorkerNode* workerNodeCopy =
                static_cast<WorkerNode*>(palloc0(sizeof(WorkerNode)));
 ...
 */
List* ActivePrimaryNonCoordinatorNodeList(LOCKMODE lockMode)
{
 ...
 * workerNodeHash.
 */
List* ActivePrimaryNodeList(LOCKMODE lockMode)
{
    EnsureModificationsCanRun();
 ...
 * workerNodeHash.
 */
List* ActivePrimaryRemoteNodeList(LOCKMODE lockMode)
{
    EnsureModificationsCanRun();
 ...
 * pg_dist_node.
 */
bool CoordinatorAddedAsWorkerNode()
{
 ...
 */
List* ReferenceTablePlacementNodeList(LOCKMODE lockMode)
{
    EnsureModificationsCanRun();
    return FilterActiveNodeListFunc(lockMode, NodeIsPrimary);
}
 ...
 * to metadata.
 */
void ErrorIfCoordinatorNotAddedAsWorkerNode()
{
    ...
    ereport(ERROR, (errmsg("operation is not allowed when coordinator "
                    ...
                    errhint("Use \"SELECT spq_set_coordinator_host('"
 ...
 */
List* DistributedTablePlacementNodeList(LOCKMODE lockMode)
{
    EnsureModificationsCanRun();
 ...
 */
bool NodeCanHaveDistTablePlacements(WorkerNode* node)
{
 ...
 */
List* ActiveReadableNonCoordinatorNodeList(void)
{
 ...
 */
List* ActiveReadableNodeList(void)
{
 ...
 */
int WorkerNodeCompare(const void* lhsKey, const void* rhsKey, Size keySize)
{
 ...
 */
uint32 WorkerNodeHashCode(const void* key, Size keySize)
{
 ...
 * WorkerPool.
 */
int NodeNamePortCompare(const char* workerLhsName, const char* workerRhsName,
 ...
 */
WorkerNode* GetFirstPrimaryWorkerNode(void)
{
