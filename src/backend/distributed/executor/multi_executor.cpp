 ...
 */
void CitusExecutorStart(QueryDesc* queryDesc, int eflags)
{
 ...
 * executes a query.
 */
void CitusExecutorRun(QueryDesc* queryDesc, ScanDirection direction, long count)
{
    if (!Session_ctx::CurrentInited()) {
    ...
    ParamListInfo savedBoundParams = Session_ctx::ExecCtx().executorBoundParams;

    /*
     * Save a pointer to query params so UDFs can access them by calling
     * ExecutorBoundParams().
     */
    Session_ctx::ExecCtx().executorBoundParams = queryDesc->params;
 ...
 */
static bool CitusCustomScanStateWalker(PlanState* planState, List** citusCustomScanStates)
{
 ...
 */
TupleTableSlot* ReturnTupleFromTuplestore(CitusScanState* scanState)
{
    ...
    List* qual = scanState->customScanState.ss.ps.qual;
    ...
    for (;;) {
        ...
        if (TupIsNull(slot)) {
             ...
             */
            if (projInfo && projInfo->pi_state.resultslot) {
 ...
 * store.
 */
void ReadFileIntoTupleStore(char* fileName, char* copyFormat, TupleDesc tupleDescriptor,
                            Tuplestorestate* tupstore)
{
    ...
    Datum* columnValues = static_cast<Datum*>(palloc0(columnCount * sizeof(Datum)));
    bool* columnNulls = static_cast<bool*>(palloc0(columnCount * sizeof(bool)));
    ...
    CopyState copyState =
        BeginCopyFrom(stubRelation, fileName, NULL, copyOptions, NULL, NULL);
    ...
    while (true) {
        ...
        bool nextRowFound = NextCopyFrom(copyState, executorExpressionContext,
                                         columnValues, columnNulls, nullptr);
        if (!nextRowFound) {
            MemoryContextSwitchTo(oldContext);
            break;
        }
 ...
 */
void SortTupleStore(CitusScanState* scanState)
{
    ...
    foreach_declared_ptr(returningEntry, targetList)
    {
        ...
        get_sort_group_operators(exprType((Node*)returningEntry->expr), true, false,
                                 false, &sortop, NULL, NULL, NULL);
    ...
    Tuplesortstate* tuplesortstate = tuplesort_begin_heap(
        tupleDescriptor, numberOfSortKeys, sortColIdx, sortOperators, collations,
        nullsFirst, u_sess->attr.attr_memory.work_mem, NULL, false);
    ...
    while (true) {
        TupleTableSlot* newSlot =
            MakeSingleTupleTableSlot(tupleDescriptor, tupleDescriptor->td_tam_ops);
        bool found = tuplesort_gettupleslot(tuplesortstate, true, newSlot, NULL);
...
static Relation StubRelation(TupleDesc tupleDescriptor)
{
    Relation stubRelation = static_cast<Relation>(palloc0(sizeof(RelationData)));
    stubRelation->rd_att = tupleDescriptor;
    stubRelation->rd_rel =
        static_cast<FormData_pg_class*>(palloc0(sizeof(FormData_pg_class)));
 ...
 */
void ExecuteQueryStringIntoDestReceiver(const char* queryString, ParamListInfo params,
                                        DestReceiver* dest)
{
 ...
 */
Query* ParseQueryString(const char* queryString, Oid* paramOids, int numParams)
{
 ...
 */
Query* RewriteRawQueryStmt(RawStmt* rawStmt, const char* queryString, Oid* paramOids,
                           int numParams)
{
    List* queryTreeList =
 ...
 * DestReceiver.
 */
void ExecuteQueryIntoDestReceiver(Query* query, ParamListInfo params, DestReceiver* dest)
{
#ifdef DISABLE_OG_COMMENTS
    int cursorOptions = CURSOR_OPT_PARALLEL_OK;
#else
    int cursorOptions = CURSOR_OPT_CUSTOM_PLAN;
#endif
 ...
 * the following:
 *      SET LOCAL spq.multi_shard_modify_mode = 'sequential';
 */
void SetLocalMultiShardModifyModeToSequential()
{
    set_config_option("spq.multi_shard_modify_mode", "sequential",
 ...
 */
void EnsureSequentialMode(ObjectType objType)
{
    ...
    ereport(DEBUG1,
            (errmsg("switching to sequential query execution mode"),
             errdetail("A command for a distributed %s is run. To make sure subsequent "
...
static bool AlterTableConstraintCheck(QueryDesc* queryDesc)
{
     ...
     * catalog tables too. For example, when dropping a column, spq_drop_trigger()
 ...
 * parameters.
 */
ParamListInfo ExecutorBoundParams(void)
{
    Assert(Session_ctx::ExecCtx().ExecutorLevel > 0);
    return Session_ctx::ExecCtx().executorBoundParams;
}
...
void EnsureTaskExecutionAllowed(bool isRemote)
{
    ...
    ereport(ERROR, (errmsg("cannot execute a distributed query from a query on a "
                    ...
                    errhint("Avoid nesting of distributed queries or use alter user "
                            "current_user set spq.allow_nested_distributed_execution "
...
static bool IsTaskExecutionAllowed(bool isRemote)
{
    if (Session_ctx::Vars().AllowNestedDistributedExecution) {
        /* user explicitly allows nested execution */
        return true;
    }

    if (!isRemote) {
        if (Session_ctx::Trans().AllowedDistributionColumnValue.isActive) {
            /*
...
static bool InLocalTaskExecutionOnShard(void)
{
    ...
    if (!DistributedTableShardId(Session_ctx::ExecCtx().LocalExecutorShardId)) {
        /*
...
static bool MaybeInRemoteTaskExecution(void)
{
    ...
    if (Session_ctx::UtilCtx().InTopLevelDelegatedFunctionCall ||
        Session_ctx::UtilCtx().InDelegatedProcedureCall) {
