 ...
 * logic in case spq.force_max_query_parallelization is enabled because we
...
/* local functions */
static DistributedExecution* CreateDistributedExecution(
    RowModifyLevel modLevel, List* taskList, ParamListInfo paramListInfo,
    int targetPoolSize, TupleDestination* defaultTupleDest,
    TransactionProperties* xactProperties, List* jobIdList, bool localExecutionSupported);
static TransactionProperties DecideTransactionPropertiesForTaskList(
...
static WorkerPool* FindOrCreateWorkerPool(DistributedExecution* execution, char* nodeName,
                                          int nodePort);
...
static bool UsingExistingSessionsCheaperThanEstablishingNewConnections(
    int readyTaskCount, WorkerPool* workerPool);
...
static void AddSessionToWaitEventSet(WorkerSession* session, WaitEventSet* waitEventSet);
...
static void ProcessWaitEvents(DistributedExecution* execution, WaitEvent* events,
                              int eventCount, bool* cancellationReceived);
 ...
 */
void AdaptiveExecutorPreExecutorRun(CitusScanState* scanState)
{
 ...
 */
TupleTableSlot* AdaptiveExecutor(CitusScanState* scanState)
{
    ...
    int targetPoolSize = Session_ctx::Vars().MaxAdaptiveExecutorPoolSize;
    List* jobIdList = NIL;
    ...
    MemoryContext localContext = AllocSetContextCreate(
        CurrentMemoryContext, "AdaptiveExecutor", ALLOCSET_DEFAULT_SIZES);
    ...
    if (RequestedForExplainAnalyze(scanState)) {
        ...
        ereport(
            ERROR,
            (errmsg("Explain (analyze) command is not supported in current version. ")));
        UseCoordinatedTransaction();
        taskList = ExplainAnalyzeTaskList(taskList, defaultTupleDest, tupleDescriptor,
                                          paramListInfo);

        /*
         * Multiple queries per task is not supported with local execution. See the Assert
         * in TupleDestDestReceiverReceive.
         */
    ...
    DistributedExecution* execution = CreateDistributedExecution(
        distributedPlan->modLevel, taskList, paramListInfo, targetPoolSize,
        defaultTupleDest, &xactProperties, jobIdList, localExecutionSupported);
    ...
    if (Session_ctx::Vars().SortReturning && distributedPlan->expectResults &&
...
static void RunLocalExecution(CitusScanState* scanState, DistributedExecution* execution)
{
    ...
    uint64 rowsProcessed = ExecuteLocalTaskListExtended(
        execution->localTaskList, estate->es_param_list_info, scanState->distributedPlan,
        execution->defaultTupleDest, isUtilityCommand);
 ...
 */
uint64 ExecuteUtilityTaskList(List* utilityTaskList, bool localExecutionSupported)
{
    RowModifyLevel modLevel = ROW_MODIFY_NONE;
    ExecutionParams* executionParams = CreateBasicExecutionParams(
        modLevel, utilityTaskList, Session_ctx::Vars().MaxAdaptiveExecutorPoolSize,
 ...
 * list for utility commands.
 */
uint64 ExecuteUtilityTaskListExtended(List* utilityTaskList, int poolSize,
                                      bool localExecutionSupported)
{
    RowModifyLevel modLevel = ROW_MODIFY_NONE;
    ExecutionParams* executionParams = CreateBasicExecutionParams(
    ...
    executionParams->xactProperties = DecideTransactionPropertiesForTaskList(
        modLevel, utilityTaskList, excludeFromXact);
    executionParams->isUtilityCommand = true;
 ...
 */
uint64 ExecuteTaskList(RowModifyLevel modLevel, List* taskList)
{
    bool localExecutionSupported = true;
    ExecutionParams* executionParams = CreateBasicExecutionParams(
        modLevel, taskList, Session_ctx::Vars().MaxAdaptiveExecutorPoolSize,
        localExecutionSupported);

    bool excludeFromXact = false;
    executionParams->xactProperties =
        DecideTransactionPropertiesForTaskList(modLevel, taskList, excludeFromXact);
 ...
 * with defaults for some of the arguments.
 */
uint64 ExecuteTaskListOutsideTransaction(RowModifyLevel modLevel, List* taskList,
                                         int targetPoolSize, List* jobIdList)
{
    /*
     * As we are going to run the tasks outside transaction, we shouldn't use local
     * execution. However, there is some problem when using local execution related to
...
/*
 * ExecuteTaskListIntoTupleDestWithParam is a proxy to ExecuteTaskListExtended() which
 * uses bind params from executor state, and with defaults for some of the arguments.
 */
uint64 ExecuteTaskListIntoTupleDestWithParam(RowModifyLevel modLevel, List* taskList,
 ...
 */
uint64 ExecuteTaskListIntoTupleDest(RowModifyLevel modLevel, List* taskList,
                                    TupleDestination* tupleDest, bool expectResults)
{
    int targetPoolSize = Session_ctx::Vars().MaxAdaptiveExecutorPoolSize;
    bool localExecutionSupported = true;
    ExecutionParams* executionParams = CreateBasicExecutionParams(
 ...
 * runs it.
 */
uint64 ExecuteTaskListExtended(ExecutionParams* executionParams)
{
    ...
    DistributedExecution* execution = CreateDistributedExecution(
        ...
        executionParams->paramListInfo, executionParams->targetPoolSize, defaultTupleDest,
        &executionParams->xactProperties, executionParams->jobIdList,
        executionParams->localExecutionSupported);
    ...
    if (executionParams->isUtilityCommand) {
        ...
        locallyProcessedRows +=
            ExecuteLocalTaskList(execution->localTaskList, defaultTupleDest);
    }
 ...
 * fields.
 */
ExecutionParams* CreateBasicExecutionParams(RowModifyLevel modLevel, List* taskList,
                                            int targetPoolSize,
                                            bool localExecutionSupported)
{
    ExecutionParams* executionParams =
        static_cast<ExecutionParams*>(palloc0(sizeof(ExecutionParams)));
    ...
    TransactionProperties* xactProperties, List* jobIdList, bool localExecutionSupported)
{
    ...
    execution->columnArray =
        static_cast<void**>(palloc0(execution->allocatedColumnCount * sizeof(void*)));
    if (Session_ctx::Vars().EnableBinaryProtocol) {
        /*
         ...
         * context.
         */
        execution->stringInfoDataArray = static_cast<StringInfoData*>(
            palloc0(execution->allocatedColumnCount * sizeof(StringInfoData)));
        for (int i = 0; i < execution->allocatedColumnCount; i++) {
            initStringInfo(&execution->stringInfoDataArray[i]);
        }
    }

    if (execution->localExecutionSupported && ShouldExecuteTasksLocally(taskList)) {
 ...
 * tables.
 */
void StartDistributedExecution(DistributedExecution* execution)
{
...
static void FinishDistributedExecution(DistributedExecution* execution)
{
    if (DistributedExecutionModifiesDatabase(execution)) {
        ...
        Session_ctx::Trans().XactModificationLevel = XACT_MODIFICATION_DATA;
    }
}

/*
 * AssignTasksToConnectionsOrWorkerPool goes through the list of tasks to determine
 * whether any task placements need to be assigned to particular connections because of
 * preceding operations in the transaction. It then adds those connections to the pool and
 * adds the task placement executions to the assigned task queue of the connection.
 */
static void AssignTasksToConnectionsOrWorkerPool(DistributedExecution* execution)
{
    ...
    foreach_declared_ptr(task, taskList)
    {
        ...
        shardCommandExecution->placementExecutions = (TaskPlacementExecution**)palloc0(
            placementExecutionCount * sizeof(TaskPlacementExecution*));
        ...
        foreach_declared_ptr(taskPlacement, task->taskPlacementList)
        {
            ...
            WorkerPool* workerPool =
                FindOrCreateWorkerPool(execution, nodeName, nodePort);
            ...
            if (execution->transactionProperties->useRemoteTransactionBlocks !=
                ...
                /*
                 * Determine whether the task has to be assigned to a particular
                 * connection due to a preceding access to the placement in the same
                 * transaction.
                 */
                                      ...
                                      ShardCommandExecution* shardCommandExecution)
{
    ...
    shardCommandExecution->attributeInputMetadata =
        static_cast<AttInMetadata**>(palloc0(queryCount * sizeof(AttInMetadata*)));
    ...
    for (uint32 queryIndex = 0; queryIndex < queryCount; queryIndex++) {
        ...
        if (tupleDescriptor == NULL) {
            attInMetadata = NULL;
        } else if (GetBm25GlobalStatQueryPrefix() == NULL &&
                   Session_ctx::Vars().EnableBinaryProtocol &&
...
static PlacementExecutionOrder ExecutionOrderForTask(RowModifyLevel modLevel, Task* task)
{
    ...
    return EXECUTION_ORDER_ANY; /* void compile error. */
}
                                          ...
                                          int nodePort)
{
    ...
    int nodeConnectionCount = Session_ctx::Vars().MaxCachedConnectionsPerWorker;
...
static void RemoteSocketClosedForAnySession(DistributedExecution* execution)
{
    ...
    int eventCount = WaitEventSetWait(execution->waitEventSet, timeout, execution->events,
                                      ...
                                      WAIT_EVENT_BUFFILE_READ /* dummy parameters*/);
...
{
    List* taskList = execution->remoteTaskList;
    int connectionMode = Session_ctx::Vars().MultiShardConnectionType;
    ...
    Session_ctx::Vars().MultiShardConnectionType = connectionMode;
}
 ...
 * has an event.
 */
void RunDistributedExecution(DistributedExecution* execution)
{
    ...
    PG_TRY();
    {
        ...
        while (!cancellationReceived &&
            ...
            int eventCount = WaitEventSetWait(
                execution->waitEventSet, timeout, execution->events,
                execution->eventSetSize, WAIT_EVENT_BUFFILE_READ /* dummy parameters*/);
...
static bool HasIncompleteConnectionEstablishment(DistributedExecution* execution)
{
    if (!Session_ctx::Vars().PreventIncompleteConnectionEstablishment) {
        return false;
    }
...
static void RebuildWaitEventSetForSessions(DistributedExecution* execution)
{
    ...
    execution->events =
        static_cast<WaitEvent*>(palloc0(execution->eventSetSize * sizeof(WaitEvent)));
            ...
            const char* errHint =
                execution->localExecutionSupported
                    ? "This command supports local execution. Consider enabling "
                      "local execution using SET spq.enable_local_execution "
                      "TO true;"
...
static bool ShouldWaitForSlowStart(WorkerPool* workerPool)
{
    ...
    if (Session_ctx::Vars().ExecutorSlowStartInterval == SLOW_START_DISABLED) {
        return false;
    }

    double milliSecondsPassedSince =
        MillisecondsPassedSince(workerPool->lastConnectionOpenTime);
    if (milliSecondsPassedSince < Session_ctx::Vars().ExecutorSlowStartInterval) {
        return true;
    }
...
static int CalculateNewConnectionCount(WorkerPool* workerPool)
{
    ...
    int activeConnectionCount PG_USED_FOR_ASSERTS_ONLY =
        workerPool->activeConnectionCount;
    int idleConnectionCount PG_USED_FOR_ASSERTS_ONLY = workerPool->idleConnectionCount;
    ...
    if (UseConnectionPerPlacement()) {
        ...
        /* If Slow start is enabled we need to update the maxNewConnection to the current
         * cycle's maximum.*/
        if (Session_ctx::Vars().ExecutorSlowStartInterval != SLOW_START_DISABLED) {
            maxNewConnectionCount =
                Min(workerPool->maxNewConnectionsPerCycle, maxNewConnectionCount);
        }
        ...
        if (Session_ctx::Vars().EnableCostBasedConnectionEstablishment &&
            newConnectionCount > 0 &&
            initiatedConnectionCount <=
                Session_ctx::Vars().MaxCachedConnectionsPerWorker &&
            UsingExistingSessionsCheaperThanEstablishingNewConnections(readyTaskCount,
                                                                       workerPool)) {
            /*
                               ...
                               TransactionProperties* transactionProperties)
{
    ...
    for (int connectionIndex = 0; connectionIndex < newConnectionCount;
         connectionIndex++) {
         ...
         * throttle connections if spq.max_shared_pool_size reached)
         */
        int adaptiveConnectionManagementFlag = AdaptiveConnectionManagementFlag(
            workerPool->poolToLocalNode, list_length(workerPool->sessionList));
...
static void CheckConnectionTimeout(WorkerPool* workerPool)
{
    ...
    if (MillisecondsBetweenTimestamps(poolStartTime, now) >=
        Session_ctx::Vars().NodeConnectionTimeout) {
        if (activeConnectionCount < requiredActiveConnectionCount) {
            ...
            if (workerPool->failureState == WORKER_POOL_FAILED_OVER_TO_LOCAL) {
                ...
                logLevel = DEBUG1;
                       ...
                       execution->failed) {
                /*
                 * The enforcement is not always erroring out. For example, if a SELECT
                 * task has two different placements, we'd warn the user, fail the pool
                 * and continue with the next placement.
                 */
                logLevel = ERROR;
            }
    ...
    foreach_declared_ptr(workerPool, execution->workerList)
    {
        ...
        if (!INSTR_TIME_IS_ZERO(workerPool->poolStartTime) &&
            ...
            long timeUntilConnectionTimeoutMs =
                Session_ctx::Vars().NodeConnectionTimeout - timeSincePoolStartMs;
        ...
        if (workerPool->readyTaskCount > UsableConnectionCount(workerPool) &&
            ...
            long timeUntilSlowStartInterval =
                Session_ctx::Vars().ExecutorSlowStartInterval - timeSinceLastConnectMs;
    ...
    do {
        ...
        switch (currentState) {
            ...
            case MULTI_CONNECTION_FAILED: {
                ...
                if (transaction->transactionCritical || execution->failed ||
...
static void HandleMultiConnectionSuccess(WorkerSession* session)
{
    ...
    ereport(
        DEBUG4,
        (errmsg("established connection to %s:%d for "
...
static bool TransactionModifiedDistributedTable(DistributedExecution* execution)
{
     ...
     * spq.function_opens_transaction_block flag. When set to false, we
        ...
        switch (currentState) {
            ...
            case REMOTE_TRANS_SENT_COMMAND: {
                ...
                if (placementExecution == NULL) {
                     ...
                     */
                    ereport(ERROR,
                            (errmsg("unable to recover from inconsistent state in "
                ...
                /*
                 * In EXPLAIN ANALYZE we need to store results except for multiple
                 * placements, regardless of query type. In other cases, doing the same
                 * doesn't seem to have a drawback.
                 */
                bool storeRows = true;
...
/*
 * PopAssignedPlacementExecution finds an executable task from the queue of assigned
 * tasks.
 */
static TaskPlacementExecution* PopAssignedPlacementExecution(WorkerSession* session)
{
    ...
    TaskPlacementExecution* placementExecution =
        dlist_container(TaskPlacementExecution, sessionReadyQueueNode,
...
static TaskPlacementExecution* PopUnassignedPlacementExecution(WorkerPool* workerPool)
{
    ...
    TaskPlacementExecution* placementExecution =
        dlist_container(TaskPlacementExecution, workerReadyQueueNode,
                        dlist_pop_head_node(readyTaskQueue));
                          ...
                          WorkerSession* session)
{
    ...
    if (hasUnresolvedParameters) {
        ...
        ExtractParametersForRemoteExecution(paramListInfo, &parameterTypes,
                                            &parameterValues);
        querySent =
            SendRemoteCommandParams(connection, queryString, parameterCount,
                                    parameterTypes, parameterValues, binaryResults);
    } else {
        /*
...
static bool ReceiveResults(WorkerSession* session, bool storeRows)
{
    ...
    TupleDestination* tupleDest =
        task->tupleDest ? task->tupleDest : execution->defaultTupleDest;
     ...
     */
    MemoryContext rowContext = AllocSetContextCreate(
        CurrentMemoryContext, "RowContext", ALLOCSET_DEFAULT_MINSIZE,
    ...
    while (!PQisBusy(connection->pgConn)) {
        ...
        if (resultStatus == PGRES_COMMAND_OK) {
            ...
            if (storeRows && *currentAffectedTupleString != '\0') {
                currentAffectedTupleCount =
                    std::strtoll(currentAffectedTupleString, NULL, 10);
        ...
        if (columnCount > execution->allocatedColumnCount) {
            ...
            execution->columnArray = static_cast<void**>(
                palloc0(execution->allocatedColumnCount * sizeof(void*)));
            if (Session_ctx::Vars().EnableBinaryProtocol) {
                /*
                ...
                execution->stringInfoDataArray = static_cast<StringInfoData*>(
                    repalloc(execution->stringInfoDataArray,
                             execution->allocatedColumnCount * sizeof(StringInfoData)));
         ...
         */
        Assert(Session_ctx::Vars().EnableBinaryProtocol || !binaryResults);
        ...
        for (uint32 rowIndex = 0; rowIndex < rowsProcessed; rowIndex++) {
            ...
            for (columnIndex = 0; columnIndex < columnCount; columnIndex++) {
                if (PQgetisnull(result, rowIndex, columnIndex)) {
                    ...
                    if (binaryResults) {
                        ...
                        appendBinaryStringInfo(&stringInfoDataArray[columnIndex], value,
                                               valueLength);
            ...
            if (binaryResults) {
                heapTuple =
                    BuildTupleFromBytes(attInMetadata, (fmStringInfo*)columnArray);
            } else {
                                   ...
                                   bool succeeded)
{
    ...
    if (succeeded) {
        ...
        uint64 durationMicrosecs = MicrosecondsBetweenTimestamps(
            placementExecution->startTime, placementExecution->endTime);
        workerPool->totalTaskExecutionTime += durationMicrosecs;
        workerPool->totalExecutedTasks += 1;

        if (IsLoggableLevel(DEBUG4)) {
            ereport(DEBUG4,
                    (errmsg("task execution (%d) for placement (%ld) on anchor "
                            ...
                            shardCommandExecution->task->anchorShardId, durationMicrosecs,
                            workerPool->nodeName, workerPool->nodePort)));
        }
...
static bool CanFailoverPlacementExecutionToLocalExecution(
    TaskPlacementExecution* placementExecution)
{
    if (!Session_ctx::Vars().EnableLocalExecution) {
...
static WaitEventSet* BuildWaitEventSet(List* sessionList)
{
    ...
    WaitEventSet* waitEventSet =
        CreateWaitEventSet(t_thrd.utils_cxt.CurrentResourceOwner, eventSetSize);
...
static void AddSessionToWaitEventSet(WorkerSession* session, WaitEventSet* waitEventSet)
{
    ...
    int waitEventSetIndex = CitusAddWaitEventSetToSet(waitEventSet, connection->waitFlags,
...
static int GetEventSetSize(List* sessionList)
{
    /* additional 1 is for latch */
    return list_length(sessionList) + 1;
}
...
static void RebuildWaitEventSetFlags(WaitEventSet* waitEventSet, List* sessionList)
{
    ...
    foreach_declared_ptr(session, sessionList)
    {
        ...
        bool success = CitusModifyWaitEvent(waitEventSet, waitEventSetIndex,
            ...
            ereport(DEBUG1,
                    (errcode(ERRCODE_CONNECTION_FAILURE),
                     errmsg("modifying wait event for node %s:%d failed. "
                            ...
                            connection->hostname, connection->port, waitEventSetIndex)));
 ...
 * the following:
 *      SET LOCAL spq.force_max_query_parallelization TO on;
 */
void SetLocalForceMaxQueryParallelization(void)
{
    set_config_option("spq.force_max_query_parallelization", "on",
