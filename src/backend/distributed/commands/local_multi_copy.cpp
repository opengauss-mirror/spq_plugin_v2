 ...
 */
void WriteTupleToLocalShard(TupleTableSlot* slot, CitusCopyDestReceiver* copyDest,
                            int64 shardId, CopyOutState localCopyOutState)
{
    ...
    if (ShouldSendCopyNow(localCopyOutState->fe_msgbuf)) {
        ...
        DoLocalCopy(localCopyOutState->fe_msgbuf, copyDest->distributedRelationId,
                    shardId, copyDest->copyStatement, isEndOfCopy,
 ...
 */
void WriteTupleToLocalFile(TupleTableSlot* slot, CitusCopyDestReceiver* copyDest,
 ...
 */
void FinishLocalCopyToShard(CitusCopyDestReceiver* copyDest, int64 shardId,
                            CopyOutState localCopyOutState)
{
    bool isBinaryCopy = localCopyOutState->binary;
    if (isBinaryCopy) {
 ...
 */
void FinishLocalCopyToFile(CopyOutState localFileCopyOutState, FileCompat* fileCompat)
{
                            ...
                            CopyOutState localCopyOutState)
{
    ...
    AppendCopyRowData(columnValues, columnNulls, copyDest->tupleDescriptor,
                      localCopyOutState, columnOutputFunctions, columnCoercionPaths);
}
...
static bool ShouldSendCopyNow(StringInfo buffer)
{
    ...
    return buffer->len > Session_ctx::Vars().LocalCopyFlushThresholdByte;
}

/*
 * DoLocalCopy finds the shard table from the distributed relation id, and copies the
 * given buffer into the shard. CopyFrom calls ReadFromLocalBufferCallback to read bytes
 * from the buffer as though it was reading from stdin. It then parses the tuples and
                        ...
                        CopyStmt* copyStatement, bool isEndOfCopy, bool isPublishable)
{
    ...
    (void)addRangeTableEntryForRelation(pState, shard, nullptr, false, false);
    CopyState cstate = BeginCopyFrom(
        shard, copyStatement->filename, copyStatement->attlist, copyStatement->options,
        &copyStatement->memUsage, t_thrd.postgres_cxt.debug_query_string,
        ReadFromLocalBufferCallback);
    CopyFrom(cstate);
    EndCopyFrom(cstate);
