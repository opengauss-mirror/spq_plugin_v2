...
static char* GenerateAlterIndexColumnSetStatsCommand(char* indexNameWithSchema,
                                                     int16 attnum, int32 attstattarget);
 ...
 * CREATE STATISTICS.
 */
List* PreprocessCreateStatisticsStmt(Node* node, const char* queryString,
 ...
 */
List* PostprocessCreateStatisticsStmt(Node* node, const char* queryString)
{
#ifdef DISABLE_OG_COMMENTS
    CreateStatsStmt* stmt = castNode(CreateStatsStmt, node);
    ...
    List* objectAddresses =
        GetObjectAddressListFromParseTree((Node*)stmt, missingOk, true);
 ...
 * was set to true.
 */
List* CreateStatisticsStmtObjectAddress(Node* node, bool missingOk, bool isPostprocess)
{
#ifdef DISABLE_OG_COMMENTS
    CreateStatsStmt* stmt = castNode(CreateStatsStmt, node);
 ...
 * DROP STATISTICS.
 */
List* PreprocessDropStatisticsStmt(Node* node, const char* queryString,
                                   ProcessUtilityContext processUtilityContext)
{
    ...
    foreach_declared_ptr(objectNameList, dropStatisticsStmt->objects)
    {
        ...
        char* ddlCommand =
            DeparseDropStatisticsStmt(objectNameList, dropStatisticsStmt->missing_ok);
 ...
 * statement.
 */
List* DropStatisticsObjectAddress(Node* node, bool missing_ok, bool isPostprocess)
{
    DropStmt* dropStatisticsStmt = castNode(DropStmt, node);
#ifdef DISABLE_OG_COMMENTS
    Assert(dropStatisticsStmt->removeType == OBJECT_STATISTIC_EXT);
 ...
 */
List* PreprocessAlterStatisticsRenameStmt(Node* node, const char* queryString,
                                          ProcessUtilityContext processUtilityContext)
{
#ifdef DISABLE_OG_COMMENTS
 ...
 */
List* PreprocessAlterStatisticsSchemaStmt(Node* node, const char* queryString,
                                          ProcessUtilityContext processUtilityContext)
{
#ifdef DISABLE_OG_COMMENTS
 ...
 */
List* PostprocessAlterStatisticsSchemaStmt(Node* node, const char* queryString)
{
#ifdef DISABLE_OG_COMMENTS
    AlterObjectSchemaStmt* stmt = castNode(AlterObjectSchemaStmt, node);
#ifdef DISABLE_OG_COMMENTS
    ...
    Oid statsOid = get_statistics_object_oid(
        list_make2(makeString(stmt->newschema), statName), false);
    Oid relationId = GetRelIdByStatsOid(statsOid);
 ...
 * was set to true.
 */
List* AlterStatisticsSchemaStmtObjectAddress(Node* node, bool missingOk,
                                             bool isPostprocess)
{
#ifdef DISABLE_OG_COMMENTS
    AlterObjectSchemaStmt* stmt = castNode(AlterObjectSchemaStmt, node);
 ...
 */
List* PreprocessAlterStatisticsStmt(Node* node, const char* queryString,
                                    ProcessUtilityContext processUtilityContext)
{
#ifdef DISABLE_OG_COMMENTS
 ...
 */
List* PreprocessAlterStatisticsOwnerStmt(Node* node, const char* queryString,
                                         ProcessUtilityContext processUtilityContext)
{
#ifdef DISABLE_OG_COMMENTS
...
/*
 * PostprocessAlterStatisticsOwnerStmt is invoked after the owner has been changed
 * locally. Since changing the owner could result in new dependencies being found for this
 * object we re-ensure all the dependencies for the statistics do exist.
 *
 * This is solely to propagate the new owner (and all its dependencies) if it was not
 * already distributed in the cluster.
 */
List* PostprocessAlterStatisticsOwnerStmt(Node* node, const char* queryString)
{
#ifdef DISABLE_OG_COMMENTS
    AlterOwnerStmt* stmt = castNode(AlterOwnerStmt, node);
#ifdef DISABLE_OG_COMMENTS
    ...
    ObjectAddress* statisticsAddress =
        static_cast<ObjectAddress*>(palloc0(sizeof(ObjectAddress)));
 ...
 */
List* GetExplicitStatisticsCommandList(Oid relationId)
{
    ...
    foreach_declared_oid(statisticsId, statisticsIdList)
    {
        ...
        Datum commandText =
            DirectFunctionCall1(pg_get_statisticsobjdef, ObjectIdGetDatum(statisticsId));
        ...
        if (DatumGetPointer(commandText) == NULL) {
            ereport(ERROR,
                    (errmsg("statistics with oid %u does not exist", statisticsId)));
        }
...
#else
    if (statisticsIdList != nullptr && list_length(statisticsIdList) > 0) {
        ereport(WARNING, (errmsg("skip GetExplicitStatisticsCommandList")));
    }
#endif
 ...
 */
List* GetExplicitStatisticsSchemaIdList(Oid relationId)
{
...
#else
    ereport(LOG, (errmsg("Skip Get dependent schema id in pg_statistic_exit, openGauss "
                         "don't have it.")));
#endif
...
List* GetAlterIndexStatisticsCommands(Oid indexOid)
{
    ...
    while (true) {
        ...
        if (targetAttstattarget != DEFAULT_STATISTICS_TARGET) {
            ...
            char* command = GenerateAlterIndexColumnSetStatsCommand(
                indexNameWithSchema, targetAttr->attnum, targetAttstattarget);

            alterIndexStatisticsCommandList = lappend(alterIndexStatisticsCommandList,
                                                     ...
                                                     int16 attnum, int32 attstattarget)
{
    ...
    appendStringInfo(&command, (ALTER_INDEX_COLUMN_SET_STATS_COMMAND),
                     indexNameWithSchema, attnum, attstattarget);
...
static char* CreateAlterCommandIfOwnerNotDefault(Oid statsOid)
{
    ...
    appendStringInfo(
        &str, "ALTER STATISTICS %s OWNER TO %s",
        NameListToQuotedString(list_make2(makeString(schemaName), makeString(statName))),
...
static char* CreateAlterCommandIfTargetNotDefault(Oid statsOid)
{
    ...
    alterStatsStmt->stxstattarget =
        getAlterStatsStxstattarget_compat(currentStxstattarget);
