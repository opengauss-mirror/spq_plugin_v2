...
#define RESET_CITUS_NODE_CONNINFO \
    "ALTER SYSTEM SET spq.node_conninfo TO 'sslmode=prefer';"
 ...
 */
#define CITUS_DEFAULT_SSL_CIPHERS    \
...
extern "C" Datum citus_setup_ssl(PG_FUNCTION_ARGS);
extern "C" Datum citus_check_defaults_for_sslmode(PG_FUNCTION_ARGS);
/*
 ...
 */
Datum citus_setup_ssl(PG_FUNCTION_ARGS)
{
#ifndef USE_SSL
    ereport(WARNING, (errmsg("can not setup ssl on postgres that is not compiled with "
                             "ssl support")));
#else  /* USE_SSL */
    if (!g_instance.attr.attr_security.EnableSSL && ShouldUseAutoSSL()) {
        ...
        if (strcmp(g_instance.attr.attr_security.SSLCipherSuites,
                   POSTGRES_DEFAULT_SSL_CIPHERS) == 0) {
            /*
 ...
 *
 * Once it is determined that the default value for spq.node_conninfo is used verbatim
 ...
 */
Datum citus_check_defaults_for_sslmode(PG_FUNCTION_ARGS)
{
    ...
    if (strcmp(NodeConninfo, "sslmode=require") == 0) {
        ...
        ereport(LOG, (errmsg("reset spq.node_conninfo to old default value as the new "
 ...
 */
static void GloballyReloadConfig()
{
 ...
 */
static bool CreateCertificatesWhenNeeded()
{
    ...
    if (SSL_CTX_use_certificate_chain_file(
            sslContext, g_instance.attr.attr_security.ssl_cert_file) == 1) {
...
static bool StoreCertificate(EVP_PKEY* privateKey, X509* certificate)
{
    const char* privateKeyFilename = g_instance.attr.attr_security.ssl_key_file;
    const char* certificateFilename = g_instance.attr.attr_security.ssl_cert_file;
